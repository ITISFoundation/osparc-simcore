...
projects_metadata = sa.Table(
    ...
    #   3. via a parent project/node that ran as a computation ("3rd generation" project, there is no limits
    #      to the number of generations)
    #
    #   in cases 2., 3. the parent_project_uuid is the direct parent project, and parent_node_id is the direct
    #   node parent as
    #   a specific node is defined by a project AND a node (since node IDs are non unique)
    #
    #   in cases 2., 3. the root_parent_project_uuid is the very first parent project, and root_parent_node_id
    #   is the very first parent node
    #
    metadata,
    sa.Column(
        "project_uuid",
    ...
    sa.Column(
        ...
        sa.String,
        nullable=True,
        doc="If applicable the parent project UUID of this project (the node that ran the public API to start "
        "this project_uuid lives in a project with UUID parent_project_uuid)",
    ),
    sa.Column(
        "parent_node_id",
        sa.String,
        nullable=True,
        doc="If applicable the parent node UUID of this project (the node that ran the public API to start "
        "this project_uuid lives in a node with ID parent_node_id)",
    ),
    sa.Column(
        "root_parent_project_uuid",
        sa.String,
        nullable=True,
        doc="If applicable the root parent project UUID of this project (the root project UUID in which the "
        "root node created the very first child)",
    ),
    sa.Column(
        "root_parent_node_id",
        sa.String,
        nullable=True,
        doc="If applicable the root parent node UUID of this project (the root node ID of the node that created "
