...
def upgrade():
    ...
    # Migrate workbench data to projects_nodes table.
    # NOTE: The workbench column is intentionally kept for now as a safety net.
    # It will be dropped in a subsequent migration once this is validated in production.
...
def downgrade():
    ...
    # Restore latest node state from projects_nodes back into projects.workbench
    # (post-upgrade writes went to projects_nodes, so workbench may be stale)
    _restore_workbench_from_projects_nodes()

    # Ensure no NULLs remain (empty projects that had no nodes)
    connection = op.get_bind()
