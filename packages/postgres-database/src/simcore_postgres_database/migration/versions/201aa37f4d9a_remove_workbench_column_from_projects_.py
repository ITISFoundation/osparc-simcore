...
def downgrade():
    ...
    # First ensure no NULLs exist (set empty workbench for rows without one)
    connection = op.get_bind()
    ...
    # Clear projects_nodes rows that were migrated from workbench and
    # leave the workbench column (still present) untouched.
