...
def downgrade():
    ...
    op.execute(f"CREATE TYPE {enum_typename} AS ENUM('CONFIRMATION_PENDING', 'ACTIVE', 'BANNED')")
    op.execute(
        f"ALTER TABLE users ALTER COLUMN {column_name} TYPE {enum_typename} USING {column_name}::text::{enum_typename}"
    )
