...
def upgrade():
    ...
    op.add_column("file_meta_data", sa.Column("upload_expires_at", sa.DateTime(), nullable=True))
    op.alter_column("file_meta_data", "file_id", existing_type=sa.VARCHAR(), nullable=False)
...
def downgrade():
    ...
    op.add_column(
        "file_meta_data",
        sa.Column("display_file_path", sa.VARCHAR(), autoincrement=False, nullable=True),
    )
