...
class ProjectNodeCreate(BaseModel):
    node_id: uuid.UUID
    required_resources: Annotated[dict[str, Any], Field(default_factory=dict)] = DEFAULT_FACTORY
    key: str
    version: str
    label: str
...
def create_workbench_subquery(project_id: str) -> Subquery:
    ...
    return (
        sa.select(
            ...
            sa.func.json_object_agg(projects_nodes.c.node_id, sa.func.json(workbench_obj)).label("workbench"),
        )
...
class ProjectNodesRepo:
    ...
    async def get(self, connection: AsyncConnection, *, node_id: uuid.UUID) -> ProjectNode:
        ...
        get_stmt = sqlalchemy.select(*[c for c in projects_nodes.c if c is not projects_nodes.c.project_uuid]).where(
