...
def assert_path_in_repo(expected_path: str, project_slug_dir: Path):
    if ":" in expected_path:
        ...
        assert any(folder_path.glob(glob_pattern)), f"no {glob_pattern=} in '{folder_path}'"
    else:
        assert (project_slug_dir / expected_path).exists(), f"'{expected_path}' is missing from '{project_slug_dir}'"
