...
def test_service_settings_base_urls(service_settings_cls: type):
    ...
    base_url = TypeAdapter(AnyHttpUrl).validate_strings(settings_with_defaults.base_url)
    api_base_url = TypeAdapter(AnyHttpUrl).validate_strings(
        settings_with_defaults.api_base_url
    )
