            ...
            async def _read_locked() -> R:
                try:
                    ...
                    async def _register_reader() -> None:
                        await client.redis.sadd(readers_set_key, reader_lock_key)

                    await _register_reader()
                    return await func(*args, **kwargs)
                finally:
                    await client.redis.srem(readers_set_key, reader_lock_key)
...
async def has_project_read_locks(
    ...
    readers_set_key = _PROJECT_REDIS_READERS_SET_KEY.format(project_uuid)
    reader_lock_keys: set[str] = await redis_client.redis.smembers(readers_set_key)
    if not reader_lock_keys:
        return False

    lock_exists = await asyncio.gather(*(redis_client.redis.exists(key) for key in reader_lock_keys))
    stale_lock_keys = [key for key, exists in zip(reader_lock_keys, lock_exists, strict=True) if not exists]
    if stale_lock_keys:
        await redis_client.redis.srem(readers_set_key, *stale_lock_keys)
