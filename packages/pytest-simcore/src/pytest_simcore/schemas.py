...
def json_schema_dict(common_schemas_specs_dir: Path) -> Iterable[Callable]:
    def schema_getter(schema_name: str) -> dict:
        ...
        assert json_file.exists(), f"Missing {schema_name} in {common_schemas_specs_dir}, please correct path"
...
@pytest.fixture(scope="session")
def random_json_from_schema(json_faker_script: Path, tmp_path_factory) -> Iterable[Callable[[str], dict[str, Any]]]:
