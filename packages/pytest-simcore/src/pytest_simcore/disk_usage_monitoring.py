...
def pytest_addoption(parser: pytest.Parser):
    ...
    simcore_group.addoption("--disk-usage", action="store_true", help="Enable disk usage monitoring")
...
class DiskUsagePlugin:
    ...
    def _log_disk_usage_increase(self, initial_usage: int, final_usage: int, scope_name: str):
        if final_usage > initial_usage:
            ...
            if increase >= self._threshold_mb:
                ...
                msg = f"Disk usage increased by {increase_mb:.2f} MB during {scope_name}."
                _logger.warning(msg)
    ...
    def monitor_module_disk_usage(self, request):
        ...
        self._log_disk_usage_increase(initial_usage, final_usage, f"the module '{module_name}'")
    ...
    def monitor_test_disk_usage(self, request):
        ...
        self._log_disk_usage_increase(initial_usage, final_usage, f"the test '{test_name}'")
