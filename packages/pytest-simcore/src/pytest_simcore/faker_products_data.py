...
@pytest.fixture
def support_email(request: pytest.FixtureRequest, product_name: ProductName) -> EmailStr:
    return TypeAdapter(EmailStr).validate_python(
        request.config.getoption("--faker-support-email", default=None) or f"support@{product_name}.info",
    )
...
def bcc_email(request: pytest.FixtureRequest, product_name: ProductName) -> EmailStr:
    return TypeAdapter(EmailStr).validate_python(
        request.config.getoption("--faker-bcc-email", default=None) or f"finance@{product_name}-department.info",
    )
