...
def test_reject_items_with_same_key():
    ...
    itemslist = DataItemsList()
    with pytest.raises(exceptions.InvalidProtocolError, message="Expecting InvalidProtocolError"):
        itemslist.insert(0, create_item("4", "int", "333"))
        itemslist.insert(0, create_item("5", "int", "333"))
        itemslist.insert(0, create_item("5", "int", "333"))

    itemslist = DataItemsList([create_item("1", "int", "333"), create_item("2", "int", "444"), create_item("3", "int", "333")])
    with pytest.raises(exceptions.InvalidProtocolError, message="Expecting InvalidProtocolError"):
        itemslist[1] = create_item("1", "int", "333")
...
def test_adding_removing_items():
    ...
    itemslist.insert(0, create_item("4", "int", "333"))
    itemslist.insert(0, create_item("5", "int", "333"))
    itemslist.insert(0, create_item("6", "int", "333"))
...
def test_read_only():
    ...
    with pytest.raises(exceptions.ReadOnlyError, message="Expecting ReadOnlyError") as excinfo:
        itemslist.insert(0, create_item("10", "int", "333"))
    assert "Trying to modify read-only object" in str(excinfo.value)

    with pytest.raises(exceptions.ReadOnlyError, message="Expecting ReadOnlyError") as excinfo:
        itemslist[1] = create_item("11", "int", "222")
    assert "Trying to modify read-only object" in str(excinfo.value)
...
def test_modifying_items_triggers_cb(): #pylint: disable=C0103
    ...
    itemslist = DataItemsList([create_item("1", "int", "333"), create_item("2", "int", "333"), create_item("3", "int", "333")], change_cb=mock_method)    
    itemslist.insert(0, create_item("10", "int", "333"))
    mock_method.assert_called_once()
    mock_method.reset_mock()
    itemslist[0] = create_item("10", "int", "4444")
    mock_method.assert_called_once()
    mock_method.reset_mock()
    itemslist[0].set(234)
    mock_method.assert_called_once()
