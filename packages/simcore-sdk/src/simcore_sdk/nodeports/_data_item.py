...
def _encode_link(node_uuid: str, link_value: str):
...
class DataItem(_DataItem):
    ...
    def set(self, value):
        ...
        if self.type in config.TYPE_TO_S3_FILE_LIST:
            ...
            new_value = _encode_link(node_uuid=node_uuid, link_value=file_path.name)
            ...
            new_value = _encode_link(node_uuid=node_uuid, link_value=folder_path.name)
    ...
    def __get_value_from_link(self):    # pylint: disable=R1710
        node_uuid, s3_object_name = _decode_link(self.value)
