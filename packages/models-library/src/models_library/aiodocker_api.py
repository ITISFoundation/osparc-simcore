...
class AioDockerContainerSpec(ContainerSpec):
    ...
    def convert_list_to_dict(cls, v):
        if v is not None and isinstance(v, list):
            converted_dict = {}
            for env in v:
                split_env = f"{env}".split("=", maxsplit=1)
                converted_dict[split_env[0]] = split_env[1] if len(split_env) > 1 else None
...
class AioDockerResources1(Resources1):
    ...
    reservation: ResourceObject | None = Field(None, description="Define resources reservation.", alias="Reservations")


class AioDockerTaskSpec(TaskSpec):
    container_spec: AioDockerContainerSpec | None = Field(default=None, alias="ContainerSpec")
