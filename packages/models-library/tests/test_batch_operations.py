...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(str, ["a", "b", "c"], ["a", "b", "c"], False, id="str_valid_no_duplicates"),
        ...
        pytest.param(str, ["a", "b", "a", "c"], ["a", "b", "c"], False, id="str_with_duplicates"),
        ...
        pytest.param(str, ["same", "same", "same"], ["same"], False, id="str_all_duplicates"),
...
def test_create_batch_ids_validator(identifier_type, input_ids, expected_output, should_raise):
    ...
    if should_raise:
        ...
        if len(set(input_ids)) != len(input_ids):  # Had duplicates
            original_first_positions = {item: input_ids.index(item) for item in set(input_ids)}
            # Items should appear in the same relative order as their first occurrence
            sorted_by_original = sorted(result, key=lambda x: original_first_positions[x])
            assert result == sorted_by_original


def test_composing_schemas_for_batch_operations(faker: Faker):
    # inner schema model
    class WebServerProjectBatchGetSchema(WebServerOutputSchema, BatchGetEnvelope[ProjectGet, ProjectID]): ...
    ...
    assert response_model.data.found_items == TypeAdapter(list[ProjectGet]).validate_python(some_projects)
