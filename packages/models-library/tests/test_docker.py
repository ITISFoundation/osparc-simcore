...
def test_simcore_service_docker_label_keys(obj_data: dict[str, Any]):
    ...
    assert all(isinstance(v, str) for v in exported_dict.values()), "docker labels must be strings!"
    assert all(key.startswith(_SIMCORE_RUNTIME_DOCKER_LABEL_PREFIX) for key in exported_dict)
    re_imported_docker_label_keys = TypeAdapter(SimcoreContainerLabels).validate_python(exported_dict)
