...
def test_constrained_str_succeeds(constraint_str_type: type[str], sample: str):
    assert TypeAdapter(constraint_str_type).validate_python(
        sample
    ) == constraint_str_type(sample)
...
def test_string_identifier_constraint_type():
    ...
    # strip spaces
    assert TypeAdapter(IDStr).validate_python("   123 trim spaces   ") == IDStr(
        "123 trim spaces"
    )

    # limited to 100!
    TypeAdapter(IDStr).validate_python("X" * 100)
    with pytest.raises(ValidationError):
        TypeAdapter(IDStr).validate_python("X" * 101)
