...
def delete_s3_prefix_permanently(client, bucket: str, prefix: str) -> tuple[int, int]:
    # NOTE: on a versioned bucket, a plain delete_objects() call (see delete_s3_prefix above) only
    # adds a delete marker -- all previous versions (and the marker itself) remain in the bucket
    # and keep consuming storage. Listing every version/delete-marker and deleting each one by its
    # VersionId removes them for good.
    ...
    for page in paginator.paginate(Bucket=bucket, Prefix=prefix):
        ...
        if not entries:
            continue
        batch = [{"Key": entry["Key"], "VersionId": entry["VersionId"]} for entry in entries]
        deleted_objects += len(batch)
        deleted_bytes += sum(int(v.get("Size", 0)) for v in versions)
        client.delete_objects(Bucket=bucket, Delete={"Objects": batch, "Quiet": True})
    return deleted_objects, deleted_bytes
...
def cleanup_bucket(
    *,
    ...
    yes: bool = typer.Option(False, "--yes", help="Skip confirmation prompt"),  # noqa: FBT003
    permanent: bool = typer.Option(
        "--permanent",
        help=(
            "Permanently delete every object version and delete marker instead of just adding a "
            "delete marker. Use this on versioned buckets to actually free up the storage."
        ),
    ),
):
    ...
    On a bucket with versioning enabled, a plain deletion only adds a delete marker and previous
    versions are kept (and still billed). Pass `--permanent` to remove all versions for good.
    """
    asyncio.run(_cleanup_bucket_async(deploy_config, report_csv, yes=yes, permanent=permanent))


async def _cleanup_bucket_async(deploy_config: Path, report_csv: Path, *, yes: bool, permanent: bool) -> None:
    deploy_config = deploy_config.expanduser()
    environment = _load_repo_config(deploy_config)
    s3cfg = _s3_config_from_env(environment, "{project_id}/")
    client = s3_client(s3cfg)
    ...
    if permanent:
        console.print(
            "[yellow]--permanent set: all object versions and delete markers will be removed for good.[/yellow]"
        )
    if not yes:
        action = "Permanently delete" if permanent else "Delete"
        confirmed = typer.confirm(f"{action} S3 data for {len(reports)} orphaned prefixes?", abort=False)
        if not confirmed:
            typer.echo("Aborted.")
            raise typer.Exit(code=0)

    delete_prefix = delete_s3_prefix_permanently if permanent else delete_s3_prefix
    deleted_objects = 0
    deleted_bytes = 0
    with _make_progress() as progress:
        task_id = progress.add_task("Deleting orphaned S3 prefixes...", total=len(reports))
        for report in reports:
            obj_count, size_count = delete_prefix(client, s3cfg.bucket, f"{report.project_id}/")
