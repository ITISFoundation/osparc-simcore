...
DEPLOY_SSH_KEY_PARSER: Final[parse.Parser] = parse.compile(r"{prefix}-{random_name}.pem")
...
DYN_SERVICES_NAMING_CONVENTION: Final[re.Pattern] = re.compile(
    r"^dy-(proxy|sidecar)(-|_)(?P<node_id>.{8}-.{4}-.{4}-.{4}-.{12}).*\t(?P<created_at>[^\t]+)\t(?P<user_id>\d+)\t(?P<project_id>.{8}-.{4}-.{4}-.{4}-.{12})\t(?P<service_name>[^\t]*)\t(?P<service_version>[^\t]*)\t(?P<product_name>[^\t]*)\t(?P<simcore_user_agent>.*)$"
)
