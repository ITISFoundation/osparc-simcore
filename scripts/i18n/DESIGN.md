...
- Make localization updates operable through a small set of make targets, one symmetric set per catalog (`backend-extract`, `backend-translate`, `backend-compile` and `frontend-extract`, `frontend-translate`). More details in [README.md](./README.md)
...
2. **Catalog (`.po`)** — the translator and reviewer work surface (human and AI). One file per language. It is kept in sync with template changes through the `backend-translate` / `frontend-translate` flow and can contain reviewed edits independent from source extraction.
3. **Binary (`.mo`)** — a compiled runtime lookup table built from `.po` for fast key lookup. It is generated by `backend-compile` and compiled when installing `common-library`, where the locale catalog lives. (Backend only — the frontend catalog is consumed as `.po` directly by `qx compile`.)

### Process Model (Four Pipeline Stages)

For the backend catalog, stages 1–2 are both run by `make backend-extract` (extraction
...
The `.mo` file is a compiled binary derived from `.po`. It is fully reproducible via `make backend-compile`, opaque in diffs, and better treated as build output.

The operational rule is simple: version-control source and catalog intent (`.py`, `.js`, `.po`), then regenerate generated artifacts through make targets (`make backend-extract`, `make backend-translate`, `make backend-compile`).
...
Partials enable scoped extraction: each service writes `_partials/service.pot`, then `make backend-extract` `msgcat`s them into the master template. You can re-scan one service without rescanning all `I18N_DIRS`, which improves iteration speed and keeps extraction boundaries explicit.
...
stateDiagram-v2
    ...
    new --> extracted : backend-extract
    extracted --> in_pot : backend-extract merge
...
flowchart TD
    ...
    PY -->|"make backend-extract"| PARTS
    PARTS -->|"make backend-extract (merge)"| POT
    POT -->|"make backend-translate"| PO
    PO -->|"make backend-compile"| MO
...
| -------- | ----------------------------------------------------------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| A        | dev adds / changes `user_message()`                                     | `make backend-extract backend-translate backend-compile`                   | or simply `make backend-all`                                                                             |
