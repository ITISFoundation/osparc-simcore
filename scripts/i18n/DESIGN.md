...
- Make localization updates operable through a small set of make targets, one symmetric set per catalog (`extract-backend`, `translate-backend`, `compile-backend` and `extract-frontend`, `translate-frontend`). More details in [README.md](./README.md)
...
2. **Catalog (`.po`)** — the translator and reviewer work surface (human and AI). One file per language. It is kept in sync with template changes through the `translate-backend` / `translate-frontend` flow and can contain reviewed edits independent from source extraction.
3. **Binary (`.mo`)** — a compiled runtime lookup table built from `.po` for fast key lookup. It is generated by `compile-backend` and compiled when installing `common-library`, where the locale catalog lives. (Backend only — the frontend catalog is consumed as `.po` directly by `qx compile`.)

### Process Model (Four Pipeline Stages)

For the backend catalog, stages 1–2 are both run by `make extract-backend` (extraction
produces the per-scope partials, then `msgcat`+enrich merges them into the master template):

1. **Extract** (`make extract-backend`) — scan `services/*` and `packages/*` source trees and generate per-scope partial templates.
2. **Merge** (`make extract-backend`) — combine those partials into the master template; existing human edits in `.po` are carried forward during the next translate sync.
3. **Translate** (`make translate-backend`) — update pending (new untranslated entries), fuzzy (source text changed), and stale (source context hash changed) entries.
4. **Compile** (`make compile-backend`) — build runtime `.mo` files from `.po` catalogs.
...
The `.mo` file is a compiled binary derived from `.po`. It is fully reproducible via `make compile-backend`, opaque in diffs, and better treated as build output.

The operational rule is simple: version-control source and catalog intent (`.py`, `.js`, `.po`), then regenerate generated artifacts through make targets (`make extract-backend`, `make translate-backend`, `make compile-backend`).

### `.po` Metadata (CTX)

#### What It Is

CTX is a source-context fingerprint added during `make extract-backend` (the `enrich` step) as `CTX-SNIPPET-VERSION`.
...
- If the version changed, the entry is treated as `stale` and is re-evaluated in `make translate-backend`.
...
If that source snippet changes in a later commit (even with the same `msgid`), `make translate-backend` can treat this entry as context-stale and refresh the translation/context.

### Why Per-Service Partials

Partials enable scoped extraction: each service writes `_partials/service.pot`, then `make extract-backend` `msgcat`s them into the master template. You can re-scan one service without rescanning all `I18N_DIRS`, which improves iteration speed and keeps extraction boundaries explicit.
...
stateDiagram-v2
    ...
    new --> extracted : extract-backend
    extracted --> in_pot : extract-backend merge
    in_pot --> pending : translate-backend sync
...
`fuzzy` — the `msgid` changed (detected during the catalog sync step in `make translate-backend`).
...
flowchart TD
    ...
    PY -->|"make extract-backend"| PARTS
    PARTS -->|"make extract-backend (merge)"| POT
    POT -->|"make translate-backend"| PO
    PO -->|"make compile-backend"| MO
...
| -------- | ----------------------------------------------------------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| A        | dev adds / changes `user_message()`                                     | `make extract-backend translate-backend compile-backend`                   | or simply `make backend`                                                                                 |
| B        | dev removes `user_message()`                                            | `make extract-backend compile-backend`                                     | `translate-backend` is safe to skip — obsolete entries are excluded automatically                        |
| C        | dev adds `@TRANSLATOR` note in source, or adds a service to `I18N_DIRS` | `make extract-backend translate-backend compile-backend`                   | note lands in `.pot` as `#.` comment; AI picks it up on next `translate-backend`                         |
| D        | translator directly edits a `.po` to fix a translation                  | `make compile-backend`                                                     | no extraction or AI step needed                                                                          |
| E        | update `glossary.json` or switch model                                  | `make translate-backend compile-backend MODEL=anthropic/claude-sonnet-4-6` | only stale/fuzzy/pending entries are re-translated; committed entries are skipped unless `USE_GIT=false` |
| F        | add a new language                                                      | `make translate-backend compile-backend LANGS=de_DE`                       | `msginit` creates the new `.po` from the existing `.pot`                                                 |
| G        | full backend rebuild                                                    | `make clean && make backend`                                               |                                                                                                          |
| —        | preview prompts without calling the LLM                                 | `make plan-backend` / `make plan-frontend`                                 | dry-run: logs the composed prompts, spends no tokens, writes nothing                                     |
| —        | frontend: string added (after `qx translate` updates `en_US.po`)        | `make translate-frontend`                                                  |                                                                                                          |
| —        | frontend: add new language                                              | `make translate-frontend CLIENT_LANGS=de_DE`                               |                                                                                                          |
