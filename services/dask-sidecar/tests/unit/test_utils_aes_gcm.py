...
def test_roundtrip_small_content(root_key: bytes, context: dict[str, str]):
    ...
    encrypted = _encrypt_to_bytes(plaintext, root_key, context)
    assert encrypted != plaintext
    assert _decrypt_to_bytes(encrypted, root_key, context) == plaintext


def test_roundtrip_empty_content(root_key: bytes, context: dict[str, str]):
    encrypted = _encrypt_to_bytes(b"", root_key, context)
    ...
    assert _decrypt_to_bytes(encrypted, root_key, context) == b""


def test_roundtrip_multichunk_large_content(root_key: bytes, context: dict[str, str]):
    chunk_size = 64
...
def test_roundtrip_final_partial_chunk(root_key: bytes, context: dict[str, str]):
    chunk_size = 100
...
def test_roundtrip_single_chunk_exact_size(root_key: bytes, context: dict[str, str]):
    chunk_size = 128
...
def test_encrypt_stream_reports_cumulative_progress(root_key: bytes, context: dict[str, str]):
    chunk_size = 64
...
def test_decrypt_stream_reports_cumulative_progress(root_key: bytes, context: dict[str, str]):
    chunk_size = 64
...
def test_encrypt_stream_progress_on_empty_input(root_key: bytes, context: dict[str, str]):
    progress: list[int] = []
    encrypt_stream(
        io.BytesIO(b""),
        io.BytesIO(),
...
def test_streams_without_progress_cb_do_not_fail(root_key: bytes, context: dict[str, str]):
    plaintext = os.urandom(200)
    encrypted = _encrypt_to_bytes(plaintext, root_key, context)
    assert _decrypt_to_bytes(encrypted, root_key, context) == plaintext


def test_two_encryptions_differ_due_to_random_seed(root_key: bytes, context: dict[str, str]):
    plaintext = b"same plaintext"
    one = _encrypt_to_bytes(plaintext, root_key, context)
    two = _encrypt_to_bytes(plaintext, root_key, context)
    assert one != two
    assert _decrypt_to_bytes(one, root_key, context) == plaintext
    assert _decrypt_to_bytes(two, root_key, context) == plaintext


def test_header_has_expected_shape(root_key: bytes, context: dict[str, str]):
    encrypted = _encrypt_to_bytes(b"abc", root_key, context)
...
def test_decrypt_rejects_tampered_magic(root_key: bytes, context: dict[str, str]):
    encrypted = bytearray(_encrypt_to_bytes(b"abc", root_key, context))
    encrypted[0] ^= 0xFF
    with pytest.raises(AesGcmStreamFormatError, match="bad magic"):
        _decrypt_to_bytes(bytes(encrypted), root_key, context)


def test_decrypt_rejects_unsupported_version(root_key: bytes, context: dict[str, str]):
    encrypted = bytearray(_encrypt_to_bytes(b"abc", root_key, context))
...
def test_decrypt_rejects_tampered_base_nonce_seed(root_key: bytes, context: dict[str, str]):
    encrypted = bytearray(_encrypt_to_bytes(b"abc", root_key, context))
...
def test_decrypt_rejects_tampered_ciphertext(root_key: bytes, context: dict[str, str]):
    encrypted = bytearray(_encrypt_to_bytes(b"abc", root_key, context))
...
def test_decrypt_rejects_chunk_larger_than_advertised_size(root_key: bytes, context: dict[str, str]):
    chunk_size = 32
    encrypted = bytearray(_encrypt_to_bytes(os.urandom(chunk_size), root_key, context, chunk_size=chunk_size))
...
def test_decrypt_rejects_tampered_tag(root_key: bytes, context: dict[str, str]):
    encrypted = bytearray(_encrypt_to_bytes(b"abc", root_key, context))
...
def test_decrypt_rejects_wrong_file_id(root_key: bytes, context: dict[str, str]):
    encrypted = _encrypt_to_bytes(b"abc", root_key, context)
    with pytest.raises(AesGcmStreamAuthError, match="authentication failed"):
        _decrypt_to_bytes(encrypted, root_key, {**context, "file_id": "other-file"})


def test_decrypt_rejects_wrong_root_key(root_key: bytes, context: dict[str, str]):
    encrypted = _encrypt_to_bytes(b"abc", root_key, context)
    with pytest.raises(AesGcmStreamAuthError, match="authentication failed"):
...
def test_encrypt_rejects_invalid_key_length(bad_key: bytes, context: dict[str, str]):
    with pytest.raises(AesGcmStreamError, match="Invalid root key"):
        _encrypt_to_bytes(b"abc", bad_key, context)


def test_encrypt_rejects_non_positive_chunk_size(root_key: bytes, context: dict[str, str]):
    with pytest.raises(AesGcmStreamError, match="Invalid chunk_size"):
        _encrypt_to_bytes(b"abc", root_key, context, chunk_size=0)


def test_decrypt_rejects_truncated_stream_missing_final_chunk(root_key: bytes, context: dict[str, str]):
    chunk_size = 32
    plaintext = os.urandom(chunk_size * 3)
    encrypted = bytearray(_encrypt_to_bytes(plaintext, root_key, context, chunk_size=chunk_size))
    ...
    with pytest.raises(AesGcmStreamAuthError, match="missing final chunk"):
        _decrypt_to_bytes(truncated, root_key, context)


def test_decrypt_rejects_trailing_data_after_final_chunk(root_key: bytes, context: dict[str, str]):
    encrypted = _encrypt_to_bytes(b"abc", root_key, context) + b"garbage"
    with pytest.raises(AesGcmStreamFormatError, match="unexpected data after final"):
        _decrypt_to_bytes(encrypted, root_key, context)


def test_decrypt_rejects_missing_header(root_key: bytes, context: dict[str, str]):
    with pytest.raises(AesGcmStreamFormatError, match="missing header"):
        _decrypt_to_bytes(b"", root_key, context)


def test_decrypt_rejects_partial_header(root_key: bytes, context: dict[str, str]):
    with pytest.raises(AesGcmStreamFormatError, match="unexpected end of input"):
        _decrypt_to_bytes(b"short", root_key, context)


def test_decrypt_rejects_zero_chunk_size_in_header(root_key: bytes, context: dict[str, str]):
    encrypted = bytearray(_encrypt_to_bytes(b"abc", root_key, context))
...
def test_decrypt_rejects_unknown_chunk_flag_bits(root_key: bytes, context: dict[str, str]):
    encrypted = bytearray(_encrypt_to_bytes(b"abc", root_key, context))
...
def test_decrypt_rejects_unsupported_header_flags(root_key: bytes, context: dict[str, str]):
    encrypted = bytearray(_encrypt_to_bytes(b"abc", root_key, context))
    # header layout: magic(8) | version(uint16) | flags(uint16) | chunk_size(uint32) | seed(12)
...
def test_decrypt_rejects_chunk_ct_len_shorter_than_tag(root_key: bytes, context: dict[str, str]):
    encrypted = bytearray(_encrypt_to_bytes(b"abc", root_key, context))
...
def test_decrypt_rejects_incomplete_chunk_ciphertext(root_key: bytes, context: dict[str, str]):
    encrypted = bytearray(_encrypt_to_bytes(b"abc", root_key, context))
...
def test_encrypt_reads_are_bounded_by_chunk_size(root_key: bytes, context: dict[str, str]):
    chunk_size = 64
...
def test_chunk_aad_matches_specification():
    ...
    expected = (
        FORMAT_MAGIC + struct.pack(">H", FORMAT_VERSION) + bytes([0b0000_0001]) + struct.pack(">Q", 7) + lp("file-abc")
    )
    assert aad == expected
...
def test_path_wrappers_roundtrip(root_key: bytes, context: dict[str, str], tmp_path: Path):
    ...
    encrypt_file(src, encrypted, root_key=root_key, chunk_size=4096, **context)
    decrypt_file(encrypted, decrypted, root_key=root_key, **context)

    assert decrypted.read_bytes() == payload


def test_path_wrappers_reject_wrong_context(root_key: bytes, context: dict[str, str], tmp_path: Path):
    src = tmp_path / "plain.bin"
    encrypted = tmp_path / "encrypted.bin"
    decrypted = tmp_path / "decrypted.bin"
