...
async def test_index_with_elements(
    ...
    await set_request_as_running(not_initialized_app, get_dynamic_service_start(uuid4()))
...
async def test_main_page(
    ...
    await assert_contains_text(async_page, "The service will be stopped and its data will be saved")
    await click_on_text(async_page, "Cancel")

    # 4. click on stop then confirm

    await assert_not_contains_text(async_page, "The service will be stopped and its data will be saved")
    await click_on_text(async_page, "Stop Service")
    await assert_contains_text(async_page, "The service will be stopped and its data will be saved")
