...
SCREENSHOTS_PATH: Final[Path] = Path(sys.argv[0] if __name__ == "__main__" else __file__).resolve().parent
...
async def _get_locator(
    ...
    async with take_screenshot_on_error(async_page):
        async for attempt in AsyncRetrying(reraise=True, wait=wait_fixed(0.1), stop=stop_after_delay(timeout)):
            with attempt:
                ...
                if instances is None:
                    ...
                    assert count == instances, f"found {count} instances of text='{text}'. Expected {instances}"
    return locator
...
def get_legacy_service_status(state: str) -> NodeGet:
    return TypeAdapter(NodeGet).validate_python(NodeGet.model_json_schema()["examples"][0] | {"service_state": state})
