...
async def enqueue_create_completed_event(app: FastAPI, schedule_id: ScheduleId) -> None:
    await _get_event_scheduler(app).enqueue_create_completed_event(schedule_id)


async def enqueue_undo_completed_event(app: FastAPI, schedule_id: ScheduleId) -> None:
    await _get_event_scheduler(app).enqueue_undo_completed_event(schedule_id)
