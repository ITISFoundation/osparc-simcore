...
class DirectorV2Service:
    ...
    @_exception_mapper(rpc_exception_map={ComputationalTaskMissingError: JobNotFoundError})
    async def get_computation_task_log_file_ids(self, *, project_id: ProjectID):
        return await computations_tasks.get_computation_task_log_file_ids(self._rpc_client, project_id=project_id)
