...
async def _celery_task_status(
    ...
    try:
        task_status = await task_manager.get_task_status(
            task_uuid=TypeAdapter(TaskUUID).validate_python(f"{job_creation_task_id}"), owner_metadata=owner_metadata
        )
        return FunctionJobCreationTaskStatus[task_status.task_state]
    except TaskNotFoundError as err:
        user_msg = f"Job creation task not found for task_uuid={
            TypeAdapter(TaskUUID).validate_python(f'{job_creation_task_id}')
        }"
        _logger.exception(
