...
def test_parse_resource_id():
    ...
    assert parse_last_resource_id(resource_name) == split_resource_name(resource_name)[-1]

    collection_to_resource_id_map = split_resource_name_as_dict(resource_name)
    # Collection-ID -> Resource-ID
    assert tuple(collection_to_resource_id_map.keys()) == parse_collections_ids(resource_name)
    assert tuple(collection_to_resource_id_map.values()) == parse_resources_ids(resource_name)
    ...
    assert collection_to_resource_id_map["jobs"] == "f622946d-fd29-35b9-a193-abdd1095167c"
