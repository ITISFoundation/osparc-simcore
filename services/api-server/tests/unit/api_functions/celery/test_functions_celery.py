...
async def _batch_register_function_jobs(
    ...
    return BatchCreateRegisteredFunctionJobs(
        created_items=[
            registered_function_job.model_copy(update={"uid": FunctionJobID(_faker.uuid4())}) for _ in function_jobs
        ]
    )
...
async def test_with_fake_run_function(
    ...
    body = {
        ...
        "input_6": [
            _faker.pyfloat(min_value=0, max_value=100) for _ in range(_faker.pyint(min_value=5, max_value=100))
        ],
    }
    ...
    mock_handler_in_functions_rpc_interface("get_function", return_value=fake_registered_project_function)
    mock_handler_in_functions_rpc_interface(
...
async def test_map_function_parent_info(
    ...
    if expected_status_code == status.HTTP_200_OK:
        ...
        task_id = patch_mock.call_args.kwargs["function_job_patch_requests"][0].patch.job_creation_task_id
...
async def test_map_function(
    ...
    async def _batch_register_function_jobs_side_effect(
        ...
        for _ in function_jobs:
            ...
            registered_jobs.append(fake_registered_project_function_job.model_copy(update={"uid": uid}))
        return BatchCreateRegisteredFunctionJobs(created_items=registered_jobs)

    mock_handler_in_functions_rpc_interface(
        "batch_register_function_jobs",
        side_effect=partial(_batch_register_function_jobs_side_effect, _generated_function_job_ids),
    )
    mock_handler_in_functions_rpc_interface(
        "get_functions_user_api_access_rights",
        return_value=FunctionUserApiAccessRights(
    ...
    for args in patch_mock.call_args_list:
        ...
        celery_task_ids = celery_task_ids.union(
            {input_.patch.job_creation_task_id for input_ in inputs if input_.patch.job_creation_task_id}
        )
