...
async def test_one_time_payment_annotations_workflow(
    app: FastAPI,
    sqlalchemy_async_engine: AsyncEngine,
):
    fake = PaymentsTransactionsDB(**PaymentsTransactionsDB.model_config["json_schema_extra"]["examples"][1])
    ...
    async with (
        ...
        insert_and_get_product_lifespan(sqlalchemy_async_engine, name=fake.product_name) as product_row,
    ):
