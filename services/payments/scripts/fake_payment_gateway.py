...
def create_payment_router():
    ...
    def init_payment(payment: InitPayment):
...
def create_payment_method_router():
    ...
    def init_payment_method(payment_method: InitPaymentMethod):
    ...
    def get_payment_method(id: PaymentMethodID):
        assert id  # nosec

    @router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT)
    def delete_payment_method(id: PaymentMethodID):
        assert id  # nosec

    @router.post("/{id}:pay", response_model=PaymentInitiated)
    def pay_with_payment_method(payment: InitPayment):
        assert payment  # nosec
