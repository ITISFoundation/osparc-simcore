...
from models_library.products import CreditResultGet, ProductStripeInfoGet
...
async def _perform_auto_recharge(
    ...
    # NOTE: PC/MD Here in the future, it may be better to have just one RPC call to the
    # web server where we get all additional data we will need, such as user email/address...
    result = await rabbitmq_rpc_client.request(
        WEBSERVER_RPC_NAMESPACE,
        parse_obj_as(RPCMethodName, "get_product_stripe_info"),
        product_name=rabbit_message.product_name,
    )
