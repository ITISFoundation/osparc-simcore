...
class ThinDV2LocalhostClient(BaseThinClient):
    ...
    async def toggle_service_observation(self, node_uuid: str, *, is_disabled: bool) -> Response:
    ...
    async def delete_service_containers(self, node_uuid: str) -> Response:
        return await self.client.delete(self._get_url(f"/services/{node_uuid}/containers"))

    @retry_on_errors()
    @expect_status(status.HTTP_202_ACCEPTED)
    async def save_service_state(self, node_uuid: str) -> Response:
        return await self.client.post(self._get_url(f"/services/{node_uuid}/state:save"))

    @retry_on_errors()
    @expect_status(status.HTTP_202_ACCEPTED)
    async def push_service_outputs(self, node_uuid: str) -> Response:
        return await self.client.post(self._get_url(f"/services/{node_uuid}/outputs:push"))

    @retry_on_errors()
    @expect_status(status.HTTP_202_ACCEPTED)
    async def delete_service_docker_resources(self, node_uuid: str) -> Response:
        return await self.client.delete(self._get_url(f"/services/{node_uuid}/docker-resources"))
    ...
    async def free_service_reserved_disk_space(self, node_uuid: str) -> Response:
        return await self.client.post(self._get_url(f"/services/{node_uuid}/disk/reserved:free"))
