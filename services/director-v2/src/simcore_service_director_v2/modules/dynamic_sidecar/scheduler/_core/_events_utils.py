...
def _extract_span_link_from_scheduler_data(scheduler_data: SchedulerData) -> Link | None:
    ...
    if not instrumentation.request_trace_carrier:
        _logger.debug(
    ...
    link = extract_span_link_from_trace_carrier(
        carrier=instrumentation.request_trace_carrier,
...
def parse_containers_inspect(
    ...
    return [
        DockerContainerInspect.from_container(containers_inspect[container_id]) for container_id in containers_inspect
    ]
...
def are_all_user_services_containers_running(
    ...
    return len(containers_inspect) > 0 and all(x.status == DockerStatus.running for x in containers_inspect)


def _get_scheduler_data(app: FastAPI, node_uuid: NodeID) -> SchedulerData:
    dynamic_sidecars_scheduler: DynamicSidecarsScheduler = app.state.dynamic_sidecar_scheduler
    # pylint: disable=protected-access
    scheduler_data: SchedulerData = dynamic_sidecars_scheduler.scheduler.get_scheduler_data(node_uuid)
...
async def service_remove_containers(
    ...
    try:
        await sidecars_client.stop_service(scheduler_data.endpoint, progress_callback=progress_callback)
    except (BaseHttpClientError, TaskExceptionError) as e:
        _logger.info(
            ("Could not remove service containers for %s. Will continue to save the data from the service! Error: %s"),
            scheduler_data.service_name,
            f"{type(e)}: {e}",
        )


async def service_free_reserved_disk_space(app: FastAPI, node_id: NodeID, sidecars_client: SidecarsClient) -> None:
...
async def service_push_outputs(
    ...
    scheduler_data: SchedulerData = _get_scheduler_data(app, node_uuid)
    await sidecars_client.push_service_output_ports(scheduler_data.endpoint, progress_callback=progress_callback)
    await sidecars_client.update_volume_state(
        scheduler_data.endpoint,
...
async def service_remove_sidecar_proxy_docker_networks_and_volumes(
    ...
    await app.state.dynamic_sidecar_scheduler.scheduler.remove_service_from_observation(scheduler_data.node_uuid)
...
async def _cleanup_long_running_tasks(app: FastAPI, service_run_id: ServiceRunID) -> None:
...
async def attempt_pod_removal_and_data_saving(app: FastAPI, scheduler_data: SchedulerData) -> None:
    # invoke container cleanup at this point
    app_settings: AppSettings = app.state.settings
    settings: DynamicServicesSchedulerSettings = app_settings.DYNAMIC_SERVICES.DYNAMIC_SCHEDULER

    _logger.debug("removing service; scheduler_data=%s", scheduler_data)

    sidecars_client: SidecarsClient = await get_sidecars_client(app, scheduler_data.node_uuid)
    ...
    await asyncio.sleep(settings.DIRECTOR_V2_DYNAMIC_SIDECAR_SLEEP_AFTER_CONTAINER_REMOVAL.total_seconds())
    ...
    if can_really_save and scheduler_data.dynamic_sidecar.were_containers_created:
        ...
        try:
            ...
            _logger.error(  # noqa: TRY400
                ("Could not contact dynamic-sidecar to save service state or output ports %s\n%s"),
                scheduler_data.service_name,
                f"{e}",
            )
...
async def attach_project_networks(app: FastAPI, scheduler_data: SchedulerData) -> None:
    ...
    projects_networks: ProjectsNetworks = await projects_networks_repository.get_projects_networks(
        project_id=scheduler_data.project_id
    )
    for (
        network_name,
...
async def wait_for_sidecar_api(app: FastAPI, scheduler_data: SchedulerData) -> None:
    ...
    try:
        async for attempt in AsyncRetrying(
            ...
            with attempt:
                if not await get_dynamic_sidecar_service_health(app, scheduler_data, with_retry=False):
                    raise TryAgain
...
async def prepare_services_environment(app: FastAPI, scheduler_data: SchedulerData) -> None:
    ...
    await sidecars_client.service_outputs_create_dirs(dynamic_sidecar_endpoint, service_outputs_labels)

    scheduler_data.dynamic_sidecar.is_service_environment_ready = True


async def get_allow_metrics_collection(app: FastAPI, user_id: UserID, product_name: ProductName) -> bool:
    ...
    if preference is None:
        return cast(bool, AllowMetricsCollectionFrontendUserPreference.get_default_value())

    allow_metrics_collection = AllowMetricsCollectionFrontendUserPreference.model_validate(preference)
