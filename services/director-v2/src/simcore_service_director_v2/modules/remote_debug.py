...
from ..core.settings import BootModeEnum
...
def setup(app: FastAPI, **_settings_kwargs):
    def on_startup() -> None:
        if app.state.settings.boot_mode == BootModeEnum.DEBUG:
            try:
            ...
            logger.info(
                "Remote debugging enabled: listening port %s", REMOTE_DEBUG_PORT
            )
