...
async def create_from_db(app: FastAPI) -> BaseCompScheduler:
    ...
    return DaskScheduler(
        ...
        db_engine=db_engine,
        default_cluster_id=app.state.settings.DIRECTOR_V2_COMPUTATIONAL_BACKEND.COMPUTATIONAL_DEFAULT_CLUSTER_ID,
        scheduled_pipelines={
            (r.user_id, r.project_uuid, r.iteration): ScheduledPipelineParams(
                cluster_id=r.cluster_id
                if r.cluster_id is not None
                else app.state.settings.DIRECTOR_V2_COMPUTATIONAL_BACKEND.COMPUTATIONAL_DEFAULT_CLUSTER_ID,
