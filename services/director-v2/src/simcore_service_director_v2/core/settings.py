...
class ComputationalBackendSettings(BaseCustomSettings):
    ...
    COMPUTATIONAL_BACKEND_PER_CLUSTER_MAX_DISTRIBUTED_CONCURRENT_CONNECTIONS: Annotated[
        ...
        Field(
            description=(
                "defines how many concurrent connections to each dask scheduler are allowed "
                "across all director-v2 replicas"
            )
        ),
    ] = 20
    ...
    COMPUTATIONAL_BACKEND_MAX_WAITING_FOR_CLUSTER_TIMEOUT: Annotated[
        ...
        Field(
            description=(
                "maximum time a pipeline can wait for a cluster to start (default to seconds, or see "
    ...
    COMPUTATIONAL_BACKEND_MAX_WAITING_FOR_RETRIEVING_RESULTS: Annotated[
        ...
        Field(
            description=(
                "maximum time the computational scheduler waits until retrieving results from the computational "
                "backend is failed (default to seconds, or see "
                "https://pydantic-docs.helpmanual.io/usage/types/#datetime-types for string formatting)."
            )
        ),
    ] = datetime.timedelta(minutes=10)
...
class AppSettings(BaseApplicationSettings, MixinLoggingSettings):
    ...
    DIRECTOR_V2_SELF_SIGNED_SSL_FILENAME: Annotated[
        ...
        Field(
            description=(
                ...
                "For background info on this special env-var see:"
    ...
    SERVICE_TRACKING_HEARTBEAT: Annotated[
        ...
        Field(
            description=(
                "Service scheduler heartbeat (everytime a heartbeat is sent into RabbitMQ) (default to seconds, or see "
                "https://pydantic-docs.helpmanual.io/usage/types/#datetime-types for string formatting)"
            )
        ),
    ] = DEFAULT_RESOURCE_USAGE_HEARTBEAT_INTERVAL
