...
def test_computation_pipeline_model_examples(model_cls: type[BaseModel], model_cls_examples: dict[str, dict[str, Any]]):
...
def test_computation_pipeline_model_with_running_state_value_field(
    ...
    for name, example in model_cls_examples.items():
        example["state"] = RunningState.WAITING_FOR_RESOURCES.value  # this is a specific Runningstate
        print(name, ":", pformat(example))
        model_instance = model_cls(**example)
        assert model_instance, f"Failed with {name}"
...
def test_computation_pipeline_model_with_uuids_in_dag_field(
    ...
    for name, example in model_cls_examples.items():
        ...
        example["dag_adjacency_list"] = {UUID(key): [UUID(n) for n in value] for key, value in old_dag_list.items()}
        print(name, ":", pformat(example))
        model_instance = model_cls(**example)
        assert model_instance, f"Failed with {name}"
