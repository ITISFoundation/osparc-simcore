...
def dynamic_services_scheduler_settings(
    setup_docker_api_proxy: None, mock_env: EnvVarsDict, monkeypatch: pytest.MonkeyPatch
) -> DynamicServicesSchedulerSettings:
...
def dynamic_sidecar_settings(
    setup_docker_api_proxy: None, mock_env: EnvVarsDict, faker: Faker, monkeypatch: pytest.MonkeyPatch
) -> DynamicSidecarSettings:
...
async def ensure_swarm_network(
    ...
    network_id = await docker_api.create_network(initialized_app, network_config)
    yield
...
async def project_id_labeled_network(
    initialized_app: FastAPI, async_docker_client: aiodocker.Docker, project_id: ProjectID
) -> AsyncIterable[str]:
    network_config = {
...
async def existing_network(
    async_docker_client: aiodocker.Docker, initialized_app: FastAPI, project_id: ProjectID
) -> AsyncIterable[str]:
...
@pytest.fixture
def initialized_app(
    dynamic_sidecar_settings: DynamicSidecarSettings, disable_postgres: None, initialized_app: FastAPI
) -> FastAPI:
...
async def test_get_swarm_network_ok(
    ...
    swarm_network = await docker_api.get_swarm_network(
        initialized_app, dynamic_services_scheduler_settings.SIMCORE_SERVICES_NETWORK_NAME
    )
...
async def test_recreate_network_multiple_times(
    ...
    network_ids = [await docker_api.create_network(initialized_app, network_config) for _ in range(10)]
...
async def test_create_service(
    ...
    service_id = await docker_api.create_service_and_get_id(initialized_app, service_spec, None)
    assert service_id
...
async def test_services_to_observe_exist(
    ...
    service_id = await docker_api.create_service_and_get_id(initialized_app, dynamic_sidecar_service_spec, None)
    assert service_id

    dynamic_services = await docker_api.get_dynamic_sidecars_to_observe(
        initialized_app, dynamic_services_scheduler_settings.SWARM_STACK_NAME
    )
...
async def test_dynamic_sidecar_in_running_state_and_node_id_is_recovered(
    ...
    dynamic_sidecar_state = await docker_api.get_dynamic_sidecar_state(initialized_app, service_id)
...
async def test_is_dynamic_sidecar_stack_missing(
    ...
    services_are_missing = await docker_api.is_dynamic_sidecar_stack_missing(
        initialized_app, node_uuid, dynamic_services_scheduler_settings.SWARM_STACK_NAME
    )
    ...
    for dynamic_sidecar_stack in dynamic_sidecar_stack_specs:
        service_id = await docker_api.create_service_and_get_id(initialized_app, dynamic_sidecar_stack, None)
        assert service_id
...
async def test_remove_dynamic_sidecar_network(
    ...
    delete_result = await docker_api.remove_dynamic_sidecar_network(initialized_app, simcore_services_network_name)
    assert delete_result is True


async def test_remove_dynamic_sidecar_network_fails(
    simcore_services_network_name: str, docker_swarm: None, initialized_app: FastAPI
):
    delete_result = await docker_api.remove_dynamic_sidecar_network(initialized_app, simcore_services_network_name)
...
async def test_get_projects_networks_containers(
    ...
    filtered_networks = (
        # pylint:disable=protected-access
        await async_docker_client.networks.docker._query_json("networks", params=params)  # noqa: SLF001
    )
...
async def test_get_or_create_networks_ids(
    test_networks: list[str], existing_network: str, project_id: ProjectID, initialized_app: FastAPI
):
...
async def test_update_scheduler_data_label(
    ...
    await docker_api.update_scheduler_data_label(initialized_app, mock_scheduler_data)
...
async def test_update_scheduler_data_label_skip_if_service_is_missing(
    async_docker_client: aiodocker.Docker, mock_scheduler_data: SchedulerData, initialized_app: FastAPI
):
...
async def test_regression_update_service_update_out_of_sequence(
    async_docker_client: aiodocker.Docker, mock_service: str, docker_swarm: None, initialized_app: FastAPI
):
...
async def test_constrain_service_to_node(
    ...
    await docker_api.constrain_service_to_node(initialized_app, mock_service, docker_node_id=target_node_id)
