...
def get_rabbitmq_settings(app: FastAPI) -> RabbitSettings:
    settings: RabbitSettings = app.state.settings.NOTIFIERS_RABBITMQ
    return settings
...
def setup_rabbitmq(app: FastAPI) -> None:
    ...
    async def _on_startup() -> None:
        ...
        app.state.rabbitmq_client = RabbitMQClient(
            client_name="notifiers", settings=settings
        )
        app.state.rabbitmq_rpc_server = await RabbitMQRPCClient.create(
            client_name="notifiers_rpc_server", settings=settings
        )
        app.state.rabbitmq_rpc_client = await RabbitMQRPCClient.create(
            client_name="notifiers_rpc_client", settings=settings
        )
