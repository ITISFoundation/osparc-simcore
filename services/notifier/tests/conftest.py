...
import pytest
import simcore_service_notifier
import yaml
...
def project_slug_dir(osparc_simcore_root_dir: Path) -> Path:
    ...
    service_folder = osparc_simcore_root_dir / "services" / "notifiers"
    assert service_folder.exists()
    assert any(service_folder.glob("src/simcore_service_notifier"))
    return service_folder


@pytest.fixture(scope="session")
def installed_package_dir() -> Path:
    dirpath = Path(simcore_service_notifier.__file__).resolve().parent
...
@pytest.fixture
def docker_compose_service_notifiers_env_vars(
    services_docker_compose_file: Path,
    env_devel_dict: EnvVarsDict,
) -> EnvVarsDict:
    """env vars injected at the docker-compose"""

    notifiers = yaml.safe_load(services_docker_compose_file.read_text())["services"][
        "notifiers"
    ]
    ...
    def _substitute(item):
        ...
        if m := re.match(r"\${([^{}:-]\w+)", value):
            ...
            try:
                ...
                pytest.fail(
                    f"{expected_env_var} is not defined in .env-devel but used in docker-compose services[{notifiers}].environment[{key}]"
                )
        return None

    envs: EnvVarsDict = {}
    for item in notifiers.get("environment", []):
...
def app_environment(
    ...
    docker_compose_service_notifiers_env_vars: EnvVarsDict,
    secret_key: str,
    fake_user_name: str,
    fake_password: str,
) -> EnvVarsDict:
    return setenvs_from_dict(
        monkeypatch,
        {
            **docker_compose_service_notifiers_env_vars,
            "NOTIFIERS_ACCESS_TOKEN_SECRET_KEY": secret_key,
            "NOTIFIERS_USERNAME": fake_user_name,
            "NOTIFIERS_PASSWORD": fake_password,
        },
    )
