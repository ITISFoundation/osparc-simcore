...
def mock_postgres_liveness(mocker: MockerFixture, test_client: TestClient) -> None:
    mocker.patch.object(test_client.app.state.postgres_liveness, "_liveness_result", new=IsNonResponsive(reason="fake"))


def test_health_postgres_unhealthy(mock_postgres_liveness: None, test_client: TestClient):
...
def mock_rabbit_healthy(mocker: MockerFixture, test_client: TestClient) -> None:
    mocker.patch.object(test_client.app.state.rabbitmq_rpc_client, "_healthy_state", new=False)
