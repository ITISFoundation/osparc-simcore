...
def test_models_are_in_sync(
    ...
    for field in class_name.__dataclass_fields__:
        assert field in metrics_definitions, f"{metrics_definitions.__qualname__} is missing {field}"
        assert hasattr(metrics_class_name, field), f"{metrics_class_name.__qualname__} is missing {field}"
