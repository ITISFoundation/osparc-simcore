...
def evaluate_service_resource_usage(starttime: datetime, stoptime:datetime, userid: int, uuid: str = '.*', image: str = 'registry.osparc.io/simcore/services/dynamic/jupyter-smash:3.0.9'):
    ...
    for currentDatetime in [stoptime - timedelta(minutes=i) for i in range(minutes)]:
        ...
        container_network_egress = prom.custom_query(
            promql_network_query, 
            params= { "time":rfc3339_str }
        )
        
        if container_cpu_seconds_usage:
            assureDictEntryExists(container_cpu_seconds_usage, max_values_per_docker_id, image, userid)
            metric_data = container_cpu_seconds_usage
            for metric in metric_data:
                current_id = metric["metric"]["id"]
                if float(metric["value"][-1]) > max_values_per_docker_id[current_id]["cpu_seconds"]:
                    max_values_per_docker_id[current_id]["cpu_seconds"] = float(metric["value"][-1])
        if container_network_egress:
            assureDictEntryExists(container_network_egress, max_values_per_docker_id, image, userid)
            metric_data = container_network_egress
            for metric in metric_data:
                current_id = metric["metric"]["id"]
                if float(metric["value"][-1]) > max_values_per_docker_id[current_id]["cpu_seconds"]:
                    max_values_per_docker_id[current_id]["egress_bytes"] = float(metric["value"][-1])
        if container_network_egress or container_cpu_seconds_usage:
            ...
            for metric in metric_data:
                ...
                simcore_service_settings = metric["metric"]["container_label_simcore_service_settings"]
                simcore_service_settings = json.loads(simcore_service_settings)
                simcore_service_settings_resources = [i for i in simcore_service_settings if "name" in i.keys() and "Resources" in i.values()][0]
                nano_cpu_limits = int(simcore_service_settings_resources['value']['Limits']['NanoCPUs'])
                max_values_per_docker_id[current_id]["nano_cpu_limits"] = nano_cpu_limits
    return max_values_per_docker_id


now = datetime.utcnow()
data = evaluate_service_resource_usage(now - timedelta(hours=1),now,userid="43817") # This userid is puppeteer1 on osparc.io
