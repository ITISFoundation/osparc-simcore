...
async def _check_node_uuid_available(client: aiodocker.docker.Docker, node_uuid: str) -> None:
    ...
    try:
        ...
        list_of_running_services_w_uuid = await client.services.list(
            filters={"label": f"{_to_simcore_runtime_docker_label_key('node_id')}={node_uuid}"}
        )
    ...
    for s in settings:
        ...
        for field in ["Source", "Target", "Type"]:
            if field in s:
                ...
                _logger.warning("Mount settings have wrong format. Required keys [Source, Target, Type]")
                continue
...
async def _read_service_settings(
    ...
    settings: dict[str, Any] | list[Any] | None = (
        json_loads(image_labels[settings_name]) if settings_name in image_labels else None
    )
...
async def _create_docker_service_params(  # noqa: C901, PLR0912, PLR0913, PLR0915
    ...
    *,
    ...
    reverse_proxy_settings = await _read_service_settings(app, service_key, service_tag, SERVICE_REVERSE_PROXY_SETTINGS)
    ...
    docker_params: dict[str, Any] = {
        "auth": (
            await _create_auth(app_settings.DIRECTOR_REGISTRY) if app_settings.DIRECTOR_REGISTRY.REGISTRY_AUTH else {}
        ),
        "registry": (
            app_settings.DIRECTOR_REGISTRY.resolved_registry_url if app_settings.DIRECTOR_REGISTRY.REGISTRY_AUTH else ""
        ),
        "name": service_name,
        "task_template": {
            "ContainerSpec": container_spec,
            "Networks": ([{"Target": internal_network_id}] if internal_network_id else []),
            "Placement": {"Constraints": ([])},
            "RestartPolicy": {
                "Condition": "on-failure",
                "Delay": app_settings.DIRECTOR_SERVICES_RESTART_POLICY_DELAY_S * pow(10, 6),
    ...
    if app_settings.DIRECTOR_OSPARC_CUSTOM_DOCKER_PLACEMENT_CONSTRAINTS:
        ...
        for (
            ...
            if resolved_value:
                ...
                docker_params["task_template"]["Placement"]["Constraints"].append(constraint)
                _logger.debug(
    ...
    if isinstance(reverse_proxy_settings, dict) and reverse_proxy_settings and reverse_proxy_settings.get("strip_path"):
        docker_params["labels"][f"traefik.http.middlewares.{service_name}_stripprefixregex.stripprefixregex.regex"] = (
            f"^/x/{node_uuid}"
        )
    ...
    placement_substitutions: dict[str, str] = app_settings.DIRECTOR_GENERIC_RESOURCE_PLACEMENT_CONSTRAINTS_SUBSTITUTIONS
    ...
    for param in service_parameters_labels:
        ...
        if param["type"] == "Resources":
            ...
            if "mem_reservation" in param["value"]:
                docker_params["task_template"]["Resources"]["Reservations"]["MemoryBytes"] = param["value"][
                    "mem_reservation"
                ]
            if "cpu_reservation" in param["value"]:
                docker_params["task_template"]["Resources"]["Reservations"]["NanoCPUs"] = param["value"][
                    "cpu_reservation"
                ]
            # REST-API compatible
            if (
                ...
                generic_resources: list = param["value"]["Reservations"]["GenericResources"]
                ...
                param["value"]["Reservations"]["GenericResources"] = [
                    x for x in generic_resources if x["DiscreteResourceSpec"]["Kind"] not in to_remove
                ]
            ...
            for resource_key in ["NanoCPUs", "MemoryBytes"]:
                ...
                resources["Reservations"][resource_key] = resources["Limits"][resource_key] = max_value

        # publishing port on the ingress network.
        elif param["name"] == "ports" and param["type"] == "int":  # backward comp
            docker_params["labels"][_to_simcore_runtime_docker_label_key("port")] = docker_params["labels"][
                f"traefik.http.services.{service_name}.loadbalancer.server.port"
            ] = str(param["value"])
        # REST-API compatible
        elif param["type"] == "EndpointSpec":
            if "Ports" in param["value"] and (
                isinstance(param["value"]["Ports"], list) and "TargetPort" in param["value"]["Ports"][0]
            ):
                docker_params["labels"][_to_simcore_runtime_docker_label_key("port")] = docker_params["labels"][
                    f"traefik.http.services.{service_name}.loadbalancer.server.port"
        ...
        # placement constraints
        elif param["name"] == "constraints" or param["type"] == "Constraints":  # python-API compatible
            ...
            if mount_settings:
                docker_params["task_template"]["ContainerSpec"]["Mounts"].extend(mount_settings)

    # add placement constraints based on what was found
    for generic_resource_kind in placement_constraints_to_substitute:
        docker_params["task_template"]["Placement"]["Constraints"] += [placement_substitutions[generic_resource_kind]]
    ...
    if constraints:
        ...
        docker_params["task_template"]["Placement"]["Constraints"] = [c.replace(" ", "") for c in constraints]
    ...
    nano_cpus_limit = str(docker_params["task_template"]["Resources"]["Limits"]["NanoCPUs"])
    mem_limit = str(docker_params["task_template"]["Resources"]["Limits"]["MemoryBytes"])
    docker_params["labels"][_to_simcore_runtime_docker_label_key("cpu_limit")] = container_spec["Labels"][
        _to_simcore_runtime_docker_label_key("cpu_limit")
    ] = f"{float(nano_cpus_limit) / 1e9}"
    docker_params["labels"][_to_simcore_runtime_docker_label_key("memory_limit")] = container_spec["Labels"][
    ...
    _logger.debug("Converted labels to docker runtime parameters: %s", pformat(docker_params))
...
async def _get_swarm_network(client: aiodocker.docker.Docker, app_settings: ApplicationSettings) -> dict:
...
async def _get_docker_image_port_mapping(
    ...
    # if empty no port is published but there might still be an internal port defined
        ...
        target_port = int(service["Spec"]["Labels"][_to_simcore_runtime_docker_label_key("port")])
...
async def _create_overlay_network_in_swarm(client: aiodocker.docker.Docker, service_name: str, node_uuid: str) -> str:
    _logger.debug("Creating overlay network for service %s with uuid %s", service_name, node_uuid)
...
async def _remove_overlay_network_of_swarm(client: aiodocker.docker.Docker, node_uuid: str) -> None:
    ...
    try:
        ...
        networks = [
            ...
            and x["Labels"][_to_simcore_runtime_docker_label_key("node_id")] == node_uuid
        ]
...
async def _get_service_state(  # noqa: C901, PLR0912
    ...
    if task_state in ("failed"):
        ...
        _logger.debug("Now is %s, time since running mode is %s", now, time_since_running)
        if time_since_running > timedelta(seconds=app_settings.DIRECTOR_SERVICES_STATE_MONITOR_S):
...
async def _get_repos_from_key(app: FastAPI, service_key: str) -> dict[str, list[str]]:
    ...
    list_of_images = {service_key: await registry_proxy.list_image_tags(app, service_key)}
...
async def _get_dependant_repos(app: FastAPI, service_key: str, service_tag: str) -> list[dict]:
    ...
    return await registry_proxy.list_interactive_service_dependencies(app, service_key, tag)
...
async def _find_service_tag(list_of_images: dict, service_key: str, service_tag: str | None) -> str:
    if service_key not in list_of_images:
        raise ServiceNotAvailableError(service_name=service_key, service_tag=service_tag)
...
async def _start_docker_service(  # noqa: PLR0913
    ...
    *,
    ...
    try:
        ...
        service_name = service["Spec"]["Name"]
        service_state, service_msg = await _get_service_state(client, dict(service), app_settings=app_settings)
        ...
        service = await client.services.inspect(service["ID"])
        published_port, target_port = await _get_docker_image_port_mapping(dict(service))
        ...
        await _silent_service_cleanup(app, node_uuid)
        raise ServiceNotAvailableError(service_name=service_key, service_tag=service_tag) from err
...
async def _create_node(  # noqa: PLR0913
    app: FastAPI,
    client: aiodocker.docker.Docker,
    user_id: str,
    project_id: str,
...
async def _get_service_key_version_from_docker_service(
    ...
    service_full_name = service_full_name[len(registry_settings.resolved_registry_url) :].strip("/")
...
async def start_service(  # noqa: PLR0913
    app: FastAPI,
    user_id: str,
    project_id: str,
    service_key: str,
    ...
    async with docker_utils.docker_client(app) as client:  # pylint: disable=not-async-context-manager
...
async def _get_node_details(app: FastAPI, client: aiodocker.docker.Docker, service: dict) -> dict:
    ...
    results = await asyncio.gather(
        _read_service_settings(app, service_key, service_tag, SERVICE_RUNTIME_BOOTSETTINGS),
    ...
    service_entrypoint = ""
    if service_boot_parameters_labels and isinstance(service_boot_parameters_labels, list):
        service_entrypoint = _get_service_entrypoint(service_boot_parameters_labels)
