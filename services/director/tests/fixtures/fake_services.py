...
def push_services(loop, docker_registry, tmpdir):
    ...
    def build_push_images(number_of_computational_services, number_of_interactive_services, inter_dependent_services=False, sleep_time_s=60, bad_json_format=False):
        try:
            version = "1.0."
            dependent_image = None
            for image_index in range(0, number_of_computational_services):
                image = _build_push_image(tmp_dir, registry_url, "computational", "test", version + str(image_index), sleep_time_s, dependent_image, bad_json_format=bad_json_format, schema_version="v1")
            ...
            for image_index in range(0, number_of_interactive_services):
                image = _build_push_image(tmp_dir, registry_url, "dynamic", "test", version + str(image_index), sleep_time_s, dependent_image, bad_json_format=bad_json_format, schema_version="v1")
                if inter_dependent_services and image_index == 0:
                    dependent_image = image
                list_of_pushed_images_tags.append(image)
...
def push_v0_schema_services(docker_registry, tmpdir):
    ...
    def build_push_images(number_of_computational_services, number_of_interactive_services, sleep_time_s=10):
        try:
            ...
            dependent_image = None
            for image_index in range(0, number_of_computational_services):
                image = _build_push_image(tmp_dir, registry_url, "computational", "test", version + str(image_index), sleep_time_s, dependent_image, schema_version=schema_version)
                list_of_pushed_images_tags.append(image)
            dependent_image = None
            for image_index in range(0, number_of_interactive_services):
                image = _build_push_image(tmp_dir, registry_url, "dynamic", "test", version + str(image_index), sleep_time_s, dependent_image, schema_version=schema_version)
                list_of_pushed_images_tags.append(image)
        except docker.errors.APIError:
            _logger.exception("Unexpected docker API error")
            raise
...
def _build_push_image(docker_dir, registry_url, service_type, name, tag, sleep_time_s, dependent_image=None, *, schema_version="v1", bad_json_format=False): # pylint: disable=R0913
    ...
    if dependent_image is not None:
        ...
        dependency_docker_labels = [{"key":dependent_description["key"], "tag":service_description["version"]}]
