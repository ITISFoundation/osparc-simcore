...
def test_list_no_services_available(docker_registry):
...
def test_list_computational_services(push_services, docker_registry ):
    push_services(6, 3)
    config.REGISTRY_URL = docker_registry
    config.REGISTRY_SSL = False
    registry_proxy.setup_registry_connection()
...
def test_list_interactive_services(docker_registry, push_services):
    push_services(5, 4)
    config.REGISTRY_URL = docker_registry
    config.REGISTRY_SSL = False
    registry_proxy.setup_registry_connection()
...
def test_retrieve_list_of_images_in_repo(docker_registry, push_services):
...
def test_retrieve_labels_of_image(docker_registry, push_services):
...
def test_get_service_details(docker_registry, push_services):
    images = push_services(1, 1)
    config.REGISTRY_URL = docker_registry
    config.REGISTRY_SSL = False
    registry_proxy.setup_registry_connection()
