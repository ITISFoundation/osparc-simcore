...
def _get_service_from_compose(compose: dict[str, Any], service_name: str) -> dict[str, Any]:
...
def test_redis_version_same_as_main_docker_compose(
    ...
    clusters_keeper_redis = _get_service_from_compose(clusters_keeper_docker_compose, "redis")
...
def test_all_services_run_on_manager_but_dask_sidecar(
    ...
    for service_name, service_config in clusters_keeper_docker_compose["services"].items():
        ...
        assert service_config["deploy"]["placement"]["constraints"] == [
            ("node.role==worker" if service_name == "dask-sidecar" else "node.role==manager")
        ]
