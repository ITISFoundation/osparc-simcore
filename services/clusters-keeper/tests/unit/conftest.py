...
def app_environment(
    ...
    envs = setenvs_from_dict(
        monkeypatch,
        {
            ...
            "PRIMARY_EC2_INSTANCES_ATTACHED_IAM_PROFILE": "",  # must be empty or we would need to add it to moto
...
def disable_clusters_management_background_task(
    ...
    start_background_task = mocker.patch(
        "simcore_service_clusters_keeper.modules.clusters_management_task.create_periodic_task",
        autospec=True,
    )

    stop_background_task = mocker.patch(
        "simcore_service_clusters_keeper.modules.clusters_management_task.cancel_wait_task",
        autospec=True,
    )
...
def create_ec2_workers(
    ...
    async def _do(num: int) -> list[str]:
        ...
        worker_name_prefix = get_cluster_name(app_settings, user_id=user_id, wallet_id=wallet_id, is_manager=False)
