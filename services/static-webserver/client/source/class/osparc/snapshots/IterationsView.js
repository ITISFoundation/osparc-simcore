...
qx.Class.define("osparc.snapshots.IterationsView", {
  ...
  members: {
    ...
    __buildLayout: function() {
      ...
      const iterationsPreview = this.__iterationPreview = new osparc.workbench.WorkbenchUIPreview();
      this.add(iterationsPreview, 1);
    },

    __buildIterations: function() {
      const loadingTable = this.__loadingTable = new osparc.snapshots.Loading(this.tr("iterations"));
        ...
        .then(iterations => {
          if (iterations.length) {
            ...
            iterations.forEach(iteration => {
              iterationPromises.push(osparc.store.Study.getOne(iteration["workcopy_project_id"]));
            });
    ...
    __reloadIteration: function(iterationId) {
      osparc.store.Study.getOne(iterationId)
        .then(data => {
