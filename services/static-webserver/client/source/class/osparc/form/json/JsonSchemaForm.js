 ...
 * @ignore(fetch)
 */
qx.Class.define("osparc.form.json.JsonSchemaForm", {
  ...
  members: {
    ...
    __expand: function(key, schema, data, depth=0, validation) {
      ...
      const container = new osparc.form.json.JsonSchemaFormItem(key, schema, depth);
      if (schema.type === "object" && schema.properties) {
        ...
        const addButton = new qx.ui.form.Button(`Add ${objectPath.get(schema, "items.title", schema.title || key)}`, "@FontAwesomeSolid/plus-circle/14");
    ...
    __expandArray: function(schema, data, depth) {
      const container = new osparc.form.json.JsonSchemaFormArray();
      // Add array items
      if (data) {
