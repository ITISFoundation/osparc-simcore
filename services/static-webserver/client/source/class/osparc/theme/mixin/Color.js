qx.Theme.define("osparc.theme.mixin.Color", {
  colors: {
    "activitytree-background-cpu": "#2C7CCE",
    "activitytree-background-memory": "#358475",

    "ready-green": "#58A6FF", // It is not really green because of reasons
    "warning-yellow": "#F8DB1F", // Speag yellow
    "busy-orange": "#FFA500",
    "failed-red": "#FF2D2D",
    "danger-red": "#E00020", // Z43 red
    "text-on-warning": "#141414",

    "info": "rgba(9, 89, 122, 1)", // #09597A
    "info-bg": "rgba(9, 89, 122, 0.1)", // #09597A1A
    "success": "rgba(10, 182, 255, 1)", // #0AB6FF
    "success-bg": "rgba(10, 182, 255, 0.1)", // #0AB6FF1A
    "warning": "rgba(255, 205, 108, 1)", // #FFCD6C
    "warning-bg": "rgba(255, 205, 108, 0.1)", // #FFCD6C1A
    "error": "rgba(255, 108, 108, 1)", // #FF6C6C
    "error-bg": "rgba(255, 108, 108, 0.1)", // #FF6C6C1A
    "disclaimer-bg": "rgba(228, 234, 237, 1)", // #E4EAEDAA

    "pb-new": "rgba(10, 182, 255, 0.25)",
    "pb-study": "rgba(10, 182, 255, 0.4)",
    "pb-template": "rgba(9, 89, 122, 0.4)",
    "pb-service": "rgba(248, 219, 31, 0.4)",
    "pb-computational": "rgba(255, 165, 0, 0.4)",
    "pb-dynamic": "rgba(248, 219, 31, 0.4)",
    "pb-locked": "rgba(113, 157, 181, 0.4)",

    // button
    "default-button-text": "rgba(255, 255, 255, 1)",
    "default-button-text-outline": "contrasted-text-light",
    "default-button-text-action": "contrasted-text-dark",
    "default-button": "product-color",
    "default-button-background": "rgba(10, 182, 255, 0.1)",
    "default-button-hover": "rgba(4, 73, 102, 1)",
    "default-button-hover-background": "rgba(10, 182, 255, 0.25)",
    "default-button-active": "rgba(9, 89, 122, 1)",
    "default-button-active-background": "rgba(4, 73, 102, 0.4)",
    "default-button-focus": "product-color",
    "default-button-focus-background": "rgba(4, 73, 102, 0.4)",
    "default-button-disabled": "text-disabled",
    "default-button-disabled-background": "rgba(113, 157, 181, 0.25)",
    "default-button-focus-blur": "rgba(254, 233, 86, 1)",

    "status_icon": "rgba(255, 255, 255, 1)",

    // separator
    "border-separator": "product-color",

    "visual-blue": "#007fd4", // Visual Studio blue

    "logger-warning-message": "warning-yellow",
    "logger-error-message": "failed-red",

    "workbench-edge-selected": "busy-orange"
  }
});
