...
def webserver_service(
    docker_stack, loop, app_config, aiohttp_unused_port, aiohttp_server, here
):
...
@pytest.mark.parametrize(
    "service_key,service_version,service_uuid",
    [
...
async def test_reverse_proxy_workflow(
    client, service_key, service_version, service_uuid
):
    """
    ...
    assert any(
        srv["key"] == service_key and srv["version"] == service_version for srv in data
    ), "version of service NOT listed in registry"

    # Start backend dynamic service ------------------------------------------------
    resp = await client.post(
        URL("/" + API_VERSION + "/running_interactive_services").with_query(
    ...
    while count < MAX_TRIALS:
        resp = await client.get(
            "/" + API_VERSION + "/running_interactive_services/{}".format(service_uuid)
        )
        ...
        await asyncio.sleep(
            WAIT_FIXED_SECS
        )  # Does this make a difference being async??
        count += 1
