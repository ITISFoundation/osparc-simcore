...
def mock_api_client_session(client, mocker):
    get_client_session = mocker.patch("simcore_service_webserver.catalog.get_client_session")
...
async def test_dag_entrypoints(client, logged_user, api_version_prefix, mock_api_client_session, expected):
    ...
    url = assert_route('replace_catalog_dag', f"/{vx}/catalog/dags/{dag_id}", dag_id=dag_id)
    ...
    # delete
    url = assert_route('delete_catalog_dag', f"/{vx}/catalog/dags/{dag_id}", dag_id=dag_id)
