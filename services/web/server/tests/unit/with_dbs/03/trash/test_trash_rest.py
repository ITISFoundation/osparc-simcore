...
async def test_trash_project_explicitly_and_empty_trash_bin(
    ...
    async for attempt in AsyncRetrying(stop=stop_after_attempt(3), wait=wait_fixed(1), reraise=True):
        with attempt:
            # NOTE: emptying trash only marks projects for immediate deletion; actual removal
            # happens exclusively via the periodic trash-pruning GC, so trigger it here.
...
async def test_trash_folder_with_subfolder_and_project_and_empty_bin(  # noqa: PLR0915
    ...
    async for attempt in AsyncRetrying(stop=stop_after_attempt(10), wait=wait_fixed(1), reraise=True):
        with attempt:
            # NOTE: emptying trash only marks contained projects for immediate deletion; actual
            # removal happens exclusively via the periodic trash-pruning GC, so trigger it here.
            await trash_service.safe_delete_expired_trash_as_admin(client.app)
