...
MOCK_FILE_UPLOAD_SCHEMA = TypeAdapter(FileUploadSchema).validate_python(
    {
        "chunk_size": "5",
        "urls": ["s3://file_id"],
        "links": {
...
MOCK_FILE_UPLOAD_COMPLETE_RESPONSE = TypeAdapter(
    FileUploadCompleteResponse
).validate_python({"links": {"state": "http://private-url"}})
...
def test_url_storage_resolver_helpers(faker: Faker, app_environment: EnvVarsDict):
    ...
    web_url: AnyUrl = _from_storage_url(
        web_request, TypeAdapter(AnyUrl).validate_python(str(storage_url))
    )
