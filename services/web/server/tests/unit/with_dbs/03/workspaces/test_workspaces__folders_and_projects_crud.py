...
@pytest.mark.parametrize("user_role,expected", [(UserRole.USER, status.HTTP_200_OK)])
async def test_workspaces_full_workflow_with_folders_and_projects2(
    client: TestClient,
    logged_user: UserInfoDict,
    user_project: ProjectDict,
    expected: HTTPStatus,
    ...
    # Create project in workspace and folder
    project_data = deepcopy(fake_project)
    project_data["workspace_id"] = f"{added_workspace['workspaceId']}"
    project_data["folder_id"] = f"{added_folder['folderId']}"
    project = await create_project(
        client.app,
        project_data,
        user_id=logged_user["id"],
    ...
    # List project in workspace and folder
    base_url = client.app.router["list_projects"].url_for()
    url = base_url.with_query(
        {
            "workspace_id": f"{added_workspace['workspaceId']}",
