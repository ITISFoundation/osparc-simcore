...
async def test_get_product_and_replace_template(
    all_products_names: list[ProductName],
    product_name: ProductName,
    logged_user: UserInfoDict,
    client: TestClient,
    ...
    # Replace product template with different content
    template_id = "some-template-id"

    response = await client.put(
        f"/v0/products/{product_name}/templates/{template_id}",
        json={"content": {"key": "value"}},
    )
    await assert_status(response, expected_status_code)
