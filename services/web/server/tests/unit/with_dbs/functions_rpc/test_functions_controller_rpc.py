...
pytest_simcore_core_services_selection = ["redis", "rabbit"]
...
# @pytest.fixture
# async def clean_functions(client: RabbitMQRPCClient):
#     # This function is a placeholder for the actual implementation
#     # that deletes all registered functions from the database.
#     functions, _ = await functions_rpc.list_functions(
#         rabbitmq_rpc_client=client, pagination_limit=100, pagination_offset=0
#     )
#     for function in functions:
#         assert function.uid is not None
#         await functions_rpc.delete_function(rabbitmq_rpc_client=client, function_id=function.uid)
...
async def test_get_function(client: RabbitMQRPCClient, mock_function: ProjectFunction):
    # Register the function first
    registered_function = await functions_rpc.register_function(
        rabbitmq_rpc_client=client, function=mock_function
    )
    assert registered_function.uid is not None

    # Retrieve the function using its ID
    retrieved_function = await functions_rpc.get_function(
        rabbitmq_rpc_client=client, function_id=registered_function.uid
    )
...
async def test_get_function_not_found(client: RabbitMQRPCClient):
    ...
    with pytest.raises(FunctionIDNotFoundError):
        await functions_rpc.get_function(
            rabbitmq_rpc_client=client, function_id=uuid4()
        )


async def test_list_functions(client: RabbitMQRPCClient):
    ...
    # List functions
    functions, _ = await functions_rpc.list_functions(
        rabbitmq_rpc_client=client, pagination_limit=10, pagination_offset=0
    )
...
async def test_list_functions_empty(client: RabbitMQRPCClient):
...
async def test_list_functions_with_pagination(
    client: RabbitMQRPCClient, mock_function: ProjectFunction
):
    ...
    functions, page_info = await functions_rpc.list_functions(
        rabbitmq_rpc_client=client, pagination_limit=2, pagination_offset=0
    )
    ...
    functions, page_info = await functions_rpc.list_functions(
        rabbitmq_rpc_client=client, pagination_limit=2, pagination_offset=2
    )
...
async def test_register_function_job(
    ...
    registered_job = await functions_rpc.register_function_job(
        rabbitmq_rpc_client=client, function_job=function_job
    )
...
async def test_get_function_job(
    ...
    retrieved_job = await functions_rpc.get_function_job(
        rabbitmq_rpc_client=client, function_job_id=registered_job.uid
    )
...
async def test_get_function_job_not_found(client: RabbitMQRPCClient):
    ...
    with pytest.raises(FunctionJobIDNotFoundError):
        await functions_rpc.get_function_job(
            rabbitmq_rpc_client=client, function_job_id=uuid4()
        )
...
async def test_function_job_collection(
    ...
    for _ in range(3):
        ...
        # Register the function job
        registered_job = await functions_rpc.register_function_job(
            rabbitmq_rpc_client=client, function_job=registered_function_job
        )
        assert registered_job.uid is not None
    ...
    registered_collection = await functions_rpc.register_function_job_collection(
        rabbitmq_rpc_client=client, function_job_collection=function_job_collection
    )
    ...
    await functions_rpc.delete_function_job_collection(
        rabbitmq_rpc_client=client, function_job_collection_id=registered_collection.uid
    )
    # Attempt to retrieve the deleted collection
    with pytest.raises(FunctionJobIDNotFoundError):
        await functions_rpc.get_function_job(
            rabbitmq_rpc_client=client, function_job_id=registered_collection.uid
        )
...
async def test_list_function_job_collections(
    ...
    collections, _ = await functions_rpc.list_function_job_collections(
        rabbitmq_rpc_client=client, pagination_limit=1, pagination_offset=1
    )
