...
async def test_create_and_delete_many_nodes_in_parallel(
    ...
    with postgres_db.connect() as conn:
        result = conn.execute(
            sa.select(projects_nodes.c.node_id).where(projects_nodes.c.project_uuid == user_project["uuid"])
        )
        assert result
...
async def test_create_many_nodes_in_parallel_still_is_limited_to_the_defined_maximum(
    ...
    with postgres_db.connect() as conn:
        result = conn.execute(
            sa.select(projects_nodes.c.node_id).where(projects_nodes.c.project_uuid == project["uuid"])
        )
        assert result
        node_ids = result.scalars().all()
