...
from simcore_service_webserver.snapshots_models import Snapshot, SnapshotPatch
...
def test_snapshot_path_autogenerated_model():

    generated_schema = SnapshotPatch.schema()

    assert generated_schema["type"] == "object"
    assert set(generated_schema["properties"].keys()) == {"name"}

    patched = SnapshotPatch(name="foo")
    assert patched.label == "foo"
