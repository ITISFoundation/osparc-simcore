...
def test_docker_composes_service_versions(osparc_simcore_root_dir: Path, here: Path):
    ...
    main_docker_compose_path = (
        osparc_simcore_root_dir / "services" / "docker-compose.yml"
    )
    main_docker_compose_specs = yaml.safe_load(main_docker_compose_path.open())
    main_services_image_names = [
        service["image"]
        for _service_name, service in main_docker_compose_specs["services"].items()
    ]
