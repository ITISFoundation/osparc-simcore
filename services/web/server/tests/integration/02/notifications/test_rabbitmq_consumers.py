...
async def test_log_workflow_only_receives_messages_if_subscribed(
    ...
    mocked_send_messages = mocker.patch(
        "simcore_service_webserver.notifications._rabbitmq_exclusive_queue_consumers.py.send_messages",
        autospec=True,
    )
...
async def test_instrumentation_workflow(
    ...
    mocked_metrics_method = mocker.patch(
        f"simcore_service_webserver.notifications._rabbitmq_nonexclusive_queue_consumers.py.{metrics_name}"
    )
