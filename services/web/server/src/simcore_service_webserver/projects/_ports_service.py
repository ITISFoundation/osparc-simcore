...
class ProjectPortData:
    ...
    def get_schema(self) -> JsonSchemaDict | None:
        ...
        if self.kind == "input" and node_meta.outputs and (input_meta := node_meta.outputs[self.io_key]):
            return self._get_port_schema(input_meta)
        if self.kind == "output" and node_meta.inputs and (output_meta := node_meta.inputs[self.io_key]):
            return self._get_port_schema(output_meta)
        return None

    def _get_port_schema(self, io_meta: ServiceInput | ServiceOutput) -> JsonSchemaDict | None:
...
def get_project_inputs(workbench: dict[NodeID, Node]) -> dict[NodeID, Any]:
    ...
    for port in iter_project_ports(workbench, "input"):
        input_to_value[port.node_id] = port.node.outputs[KeyIDStr("out_1")] if port.node.outputs else None
    return input_to_value


def set_inputs_in_project(workbench: dict[NodeID, Node], update: dict[NodeID, Any]) -> set[NodeID]:
...
class _OutputPortInfo(NamedTuple):
    ...
    task_output_in_workbench: Any  # the value in the workbench of a computational node
    ...
    for port in iter_project_ports(workbench, "output"):
        if port.node.inputs:
            try:
                ...
                port_link = _NonStrictPortLink.model_validate(port.node.inputs[KeyIDStr("in_1")])
                ...
                output_to_value[port.node_id] = _OutputPortInfo(
                    ...
                    task_output_in_workbench=(
                        ...
                        task_node.outputs.get(task_output_name) if task_node.outputs is not None else None
                    ),
                )
...
async def _get_computation_tasks_outputs(
    ...
    batch: TasksOutputs = await get_batch_tasks_outputs(app, project_id=project_id, selection=selection)
...
async def get_project_outputs(
    ...
    tasks_outputs = await _get_computation_tasks_outputs(app, project_id=project_id, nodes_ids=task_node_ids)
