...
from . import _core_get, _core_nodes
...
async def create_node(request: web.Request) -> web.Response:
    ...
    try:
        ...
        data = {
            "node_id": await _core_nodes.add_project_node(
                request,
                project_uuid,
                user_id,
                body["service_key"],
...
async def get_node(request: web.Request) -> web.Response:
    ...
    try:
        ...
        reply: Union[Dict, List] = await director_v2_api.get_dynamic_service_state(
...
async def post_retrieve(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        await director_v2_api.retrieve_dynamic_service_inputs(
            request.app, node_uuid, port_keys
        ),
        dumps=json_dumps,
    )
...
async def post_restart(request: web.Request) -> web.Response:
    ...
    await director_v2_api.restart_dynamic_service(request.app, node_uuid)
...
async def delete_node(request: web.Request) -> web.Response:
    ...
    try:
        ...
        await _core_nodes.delete_project_node(request, project_uuid, user_id, node_uuid)
