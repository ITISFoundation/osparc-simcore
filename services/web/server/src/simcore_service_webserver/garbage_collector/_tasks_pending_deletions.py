"""Periodic task that drives the `projects_pending_deletion` outbox.

See `simcore_service_webserver.projects._pending_deletion_service` for the
single-pass logic.
"""
...
from ..projects import _pending_deletion_service
...
def create_background_task_to_retry_pending_deletions(
    ...
    async def _cleanup_ctx_fun(app: web.Application) -> AsyncIterator[None]:
        ...
        async def _retry_pending_deletions_periodically() -> None:
            with log_context(_logger, logging.INFO, "Retrying pending project deletions"):
                await _pending_deletion_service.retry_pending_deletions(app)

        async for _ in periodic_task_lifespan(app, _retry_pending_deletions_periodically):
            yield
