...
async def _remove_service(app: web.Application, node_id: NodeID, service: DynamicServiceGet) -> None:
    ...
    if not await is_node_id_present_in_any_project_workbench(app, node_id):
        ...
        try:
            user_role: UserRole = await users_service.get_user_role(app, user_id=service.user_id)
        except (UserNotFoundError, ValueError):
            save_service_state = False
        else:
            save_service_state = user_role > UserRole.GUEST and await has_user_project_access_rights(
                app,
...
async def remove_orphaned_services(registry: RedisResourceRegistry, app: web.Application) -> None:
    ...
    # NOTE: First get the running services in the system before checking what should be opened
    ...
    # Why? If a service is running but the nodes form the corresponding project cannot be listed,
    ...
    async for project_nodes_future in limited_as_completed(
        (list_node_ids_in_project(app, project_id) for project_id in known_opened_project_ids),
        ...
        try:
            ...
            _logger.warning(
                create_troubleshooting_log_kwargs(
                    ("Skipping orpahn services removal, call to `list_node_ids_in_project` raised"),
                    error=e,
                    error_context={
    ...
    orphaned_running_service_ids = set(running_services_by_id) - potentially_running_service_ids_set
