...
class RRIDEntry(BaseModel):
    rrid_term: Annotated[StrictStr, Field(description="Associated tools or resources used")]
    ...
    ontological_term: Annotated[StrictStr, Field(description="Associated ontological term (human-readable)")] = ""
    ontological_identifier: Annotated[
        StrictStr,
        Field(description="Associated ontological identifier from SciCrunch https://scicrunch.org/sawg"),
    ] = ""
...
class InputsEntryModel(BaseModel):
    service_alias: Annotated[
        StrictStr,
        Field(description="Name of the service containing this input, given by the user"),
    ]
    service_name: Annotated[StrictStr, Field(description="Name of the service containing this input")]
    service_key: Annotated[ServiceKey, Field(description="Key of the service containing this input")]
    service_version: Annotated[
        ServiceVersion,
        Field(description="Version of the service containing this input"),
    ]
    input_name: Annotated[StrictStr, Field(description="An input field to the MSoP submission")] = ""
    input_parameter_description: Annotated[
        StrictStr, Field(description="Description of what the parameter represents")
    ] = ""
    input_data_type: Annotated[StrictStr, Field(description="Data type for the input field (in plain text)")] = ""
    input_data_units: Annotated[StrictStr, Field(description="Units of data for the input field, if applicable")] = ""
    input_data_default_value: Annotated[
        StrictStr,
        Field(description="Default value for the input field, if applicable (doi or value)"),
    ] = ""
    input_data_constraints: Annotated[
        StrictStr,
        Field(description="Range [min, max] of acceptable parameter values, or other constraints as formulas / sets"),
    ] = ""
...
class OutputsEntryModel(BaseModel):
    service_alias: Annotated[
        StrictStr,
        Field(description="Name of the service producing this output, given by the user"),
    ]
    service_name: Annotated[StrictStr, Field(description="Name of the service containing this output")]
    service_key: Annotated[ServiceKey, Field(description="Key of the service containing this output")]
    service_version: Annotated[
        ServiceVersion,
        Field(description="Version of the service containing this output"),
    ]
    output_name: Annotated[StrictStr, Field(description="An output field to the MSoP submission")] = ""
    ...
    output_data_type: Annotated[StrictStr, Field(description="Data type for the output field")] = ""
...
class CodeDescriptionParams(BaseModel):
    code_description: Annotated[CodeDescriptionModel, Field(description="code description data")]
    inputs: Annotated[
...
class BaseSheetDivisionParts(BaseModel):
    ...
    @abstractmethod
    def get_cell_styles(self, o: int, sheet_data: BaseModel) -> list[tuple[str, BaseXLSXCellData]]:
...
class RRIDSheetPart(BaseSheetDivisionParts):
    ...
    def get_cell_styles(self, o: int, sheet_data: BaseModel) -> list[tuple[str, BaseXLSXCellData]]:
        static_cells: list[tuple[str, BaseXLSXCellData]] = [
            ...
            (
                f"B{o + 4}",
                T("Ontology term (human-readable)  associated with the project submission"),
            ),
            (
                f"B{o + 5}",
                Link(
        ...
        # add data to this
        rrid_entries: list[RRIDEntry] = cast(list[RRIDEntry], sheet_data)

        rrid_cells: list[tuple[str, BaseXLSXCellData]] = []
        for column_letter, (i, rrid_entry) in zip(
            column_generator(4, len(rrid_entries)), enumerate(rrid_entries), strict=True
        ):
            rrid_cells.append(
                (
                    f"{column_letter}{o + 1}",
            ...
            rrid_cells.append((f"{column_letter}{o + 5}", T(rrid_entry.ontological_identifier)))
        ...
        for column_letter in column_generator(3, len(rrid_entries) + 1):
...
class TSRSheetPart(BaseSheetDivisionParts):
    ...
    def get_cell_styles(self, o: int, sheet_data: BaseModel) -> list[tuple[str, BaseXLSXCellData]]:
        static_cells: list[tuple[str, BaseXLSXCellData]] = [
            ...
            (
                f"B{o + 2}",
                T("Column type. Valid values are: Link, Text, Rating, Target, Target Justification")
                | Comment(
                    (
            ...
            (
                f"B{o + 4}",
                T("Links to data that was used to create, validate, test, etc. the project."),
            ),
            ...
            (
                f"B{o + 6}",
                T("Link to test results from running the tests for the project, e.g., on the o²S²PARC platform"),
            ),
            ...
            (
                f"B{o + 8}",
                T("Link to documentation of known issues and limitations of the project."),
            ),
            ...
            (
                f"B{o + 11}",
                T("Link to the download or release page for the project, e.g., on the SPARC Portal."),
            ),
            ...
            (
                f"B{o + 14}",
                T("Link to language ecosystem package repository. For example, PyPI for python projects."),
            ),
            (f"C{o + 14}", T("https://pypi.org/project/sparcur/")),
            (f"A{o + 15}", T("TSR7e: Broad Dissemination: Interactive")),
            (
                f"B{o + 15}",
                T("Link to the project on an interactive software hosting platform e.g. o²S²PARC"),
            ),
            (
                f"C{o + 15}",
                T("https://github.com/tgbugs/dockerfiles/blob/master/source.org#sparcron-user"),
            ),
            (f"A{o + 16}", T("TSR8a: Independent Reviews")),
            (
                f"B{o + 16}",
                T("Links to reviews of project by independent members of the community."),
            ),
            ...
            (
                f"B{o + 18}",
                T("Link to benchmarking against other projects operating in the same domain."),
            ),
            (f"A{o + 19}", T("TSR10a: Relevant standards")),
            (
                f"B{o + 19}",
                T("List and/or link to standards/guidelines that this project conforms to or implements."),
            ),
        ...
        max_references_length = max(len(tsr_entries[k].references) for k in _SORTED_REFERENCE_ITEMS)
        ...
        for i, key in enumerate(_SORTED_REFERENCE_ITEMS):
            ...
            for c, reference in zip(column_generator(7, len(references)), references, strict=True):
...
class InputsOutputsSheetPart(BaseSheetDivisionParts):
    ...
    def get_cell_styles(self, o: int, sheet_data: BaseModel) -> list[tuple[str, BaseXLSXCellData]]:
        static_cells: list[tuple[str, BaseXLSXCellData]] = [
            ...
            (
                f"B{o + 4}",
                T("Type (bool, int/enum, real, complex/phasor, vector, array, table, field, structure, file, other)")
                | Backgrounds.green,
            ),
            (
                f"B{o + 5}",
            ...
            (
                f"B{o + 9}",
                T("Range [min, max] of acceptable parameter values, or other constraints as formulas / sets")
                | Backgrounds.green,
            ),
            (
                f"B{o + 10}",
            ...
            (
                f"B{o + 14}",
                T("Range [min, max] of possible output values, or other constraints as formulas / sets")
        ...
        code_description_params: CodeDescriptionParams = cast(CodeDescriptionParams, sheet_data)

        # formatted inputs

        inputs: list[InputsEntryModel] = [
            x for x in code_description_params.inputs if _include_ports_from_this_service(x.service_key)
        ]
        ...
        for column_letter, input_entry in zip(column_generator(4, len(inputs)), inputs, strict=True):
            inputs_cells.append((f"{column_letter}{o + 3}", T(input_entry.input_name)))
            inputs_cells.append((f"{column_letter}{o + 4}", T(input_entry.input_data_type)))
            inputs_cells.append((f"{column_letter}{o + 5}", T(input_entry.input_parameter_description)))
            inputs_cells.append((f"{column_letter}{o + 6}", T(input_entry.input_data_units)))
            inputs_cells.append((f"{column_letter}{o + 7}", T(input_entry.input_data_default_value)))
            inputs_cells.append((f"{column_letter}{o + 8}", T(input_entry.input_data_constraints)))

        # formatted outputs

        outputs: list[OutputsEntryModel] = [
            x for x in code_description_params.outputs if _include_ports_from_this_service(x.service_key)
        ]

        outputs_cells: list[tuple[str, BaseXLSXCellData]] = [(f"D{o + 9}", T(len(outputs)))]

        for column_letter, output_entry in zip(column_generator(4, len(outputs)), outputs, strict=True):
            inputs_cells.append((f"{column_letter}{o + 10}", T(output_entry.output_name)))
            inputs_cells.append((f"{column_letter}{o + 11}", T(output_entry.output_data_type)))
            inputs_cells.append((f"{column_letter}{o + 12}", T(output_entry.output_parameter_description)))
            inputs_cells.append((f"{column_letter}{o + 13}", T(output_entry.output_data_units)))
            inputs_cells.append((f"{column_letter}{o + 13}", T(output_entry.output_data_constraints)))

        # write top "value n" labels
        value_labels: list[tuple[str, BaseXLSXCellData]] = []
        for i, column_letter in enumerate(column_generator(4, max(len(inputs), len(outputs)))):
            value_labels.append(
                (
                    f"{column_letter}{o + 1}",
...
class SheetCodeDescriptionV2(BaseXLSXSheet):
    ...
    def assemble_data_for_template(self, template_data: BaseModel | None) -> list[tuple[str, BaseXLSXCellData]]:
        code_description_params: CodeDescriptionParams = ensure_correct_instance(template_data, CodeDescriptionParams)
        ...
        entries: list[tuple[BaseSheetDivisionParts, Any]] = [
            (RRIDSheetPart(), code_description_params.code_description.rrid_entries),
