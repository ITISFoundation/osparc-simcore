...
class RRIDSheetPart(BaseSheetDivisionParts):
    ...
    def get_cell_styles(
        ...
        for column_letter, (i, rrid_entry) in zip(
            ...
            rrid_cells.append(
                (f"{column_letter}{o + 4}", T(rrid_entry.ontological_term))
            )
            rrid_cells.append(
...
class TSRSheetPart(BaseSheetDivisionParts):
    ...
    def get_cell_styles(
        ...
        static_cells: list[tuple[str, BaseXLSXCellData]] = [
            ...
            (
                f"A{o + 20}",
                T("Link to demonstration that project conforms to standard."),
            ),
        ]
        ...
        link_labels_cells: list[tuple[str, BaseXLSXCellData]] = [
            (f"{c}{o + 2}", T("Link"))
            for c in column_generator(7, max_references_length)
        ]
        ...
        items_already_converted_cells: list[tuple[str, BaseXLSXCellData]] = [
            (
                f"D{o + 6}",
                T("All TSR3 items are covered by the rating on the TSR3a row"),
            ),
...
class InputsOutputsSheetPart(BaseSheetDivisionParts):
    ...
    def get_cell_styles(
        ...
        static_cells: list[tuple[str, BaseXLSXCellData]] = [
            ...
            (
                f"B{o + 7}",
                T("Default value for the parameter") | Backgrounds.yellow_dark,
            ),
        ...
        for column_letter, output_entry in zip(
            ...
            inputs_cells.append(
                (f"{column_letter}{o + 10}", T(output_entry.output_name))
            )
            inputs_cells.append(
            ...
            inputs_cells.append(
                (
                    f"{column_letter}{o + 12}",
                    T(output_entry.output_parameter_description),
                )
            )
            inputs_cells.append(
