    ...
    *,
    ...
    async with transaction_context(get_asyncpg_engine(app), connection) as conn:
        stmt = api_keys.delete().where(
            (api_keys.c.id == int(api_key_id))  # NOTE See: https://github.com/ITISFoundation/osparc-simcore/issues/6919
...
async def delete_expired_api_keys(app: web.Application, connection: AsyncConnection | None = None) -> list[str]:
    async with transaction_context(get_asyncpg_engine(app), connection) as conn:
        stmt = (
            api_keys.delete()
            .where((api_keys.c.expires_at.is_not(None)) & (api_keys.c.expires_at < sa.func.now()))
    ...
    *,
    ...
    async with pass_or_acquire_connection(get_asyncpg_engine(app), connection) as conn:
        ...
        if not include_autogenerated:
            stmt = stmt.where(~api_keys.c.api_key.like(f"{API_KEY_AUTOGENERATED_KEY_PREFIX}%"))
