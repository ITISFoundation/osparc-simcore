...
def create_redirect_response(app: web.Application, page: str, **parameters) -> web.HTTPFound:
    """
...
class QueryParams(BaseModel, ValidationMixin):
    ...
    async def check_download_link(self):
        ...
        try:
            ...
            log.debug("Invalid download link '%s'. If failed fetch check with %s", self.download_link, err)
            raise web.HTTPBadRequest(reason="The download link provided is invalid") from err
