...
def _create_project_with_filepicker(
    project_id: ProjectID,
    ...
    file_picker, _ = _create_file_picker(download_link)
...
async def get_or_create_project_with_file(
    ...
    *,
    ...
    project_uid: ProjectID = compose_uuid_from(
        user.id, params.footprint
    )  # FIXME: does not identify file
    ...
    if not await _project_exists(
        ...
        # nodes
        file_picker, _ = _create_file_picker(params.download_link)

        # project
        project = _create_project(
            project_id=project_uid,
            name=f"File picker {params.file_name}.{params.file_type}",
            description=f"Autogenerated study with a file-picker {params.footprint}",
            thumbnail=thumbnail,
            owner=user,
            workbench={  # type: ignore
                f"{file_picker_id}": file_picker,
