...
class PaymentsAutorechargeDB(BaseModel):
    ...
    inc_payment_amount_in_usd: NonNegativeDecimal
    inc_payment_countdown: PositiveInt | None
...
async def replace_wallet_autorecharge(
    ...
    *,
    ...
    async with get_database_engine(app).acquire() as conn:
        stmt = Statements.is_valid_payment_method(
            user_id=user_id,
        ...
        # TODO:
        stmt = Statements.update_wallet_autorecharge(
            wallet_id,
        )
        result = await conn.execute(stmt)
        row = await result.first()
