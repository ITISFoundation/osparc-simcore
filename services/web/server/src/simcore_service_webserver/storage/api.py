...
async def list_storage_locations(app: web.Application, user_id: UserID) -> FileLocationArray:
...
async def get_project_total_size_simcore_s3(app: web.Application, user_id: UserID, project_uuid: ProjectID) -> ByteSize:
    with log_context(
        _logger,
        logging.DEBUG,
        ...
        for location in user_accessible_locations:
            files_metadata_url = (api_endpoint / "locations" / f"{location.id}" / "files" / "metadata").with_query(
                user_id=user_id, project_id=f"{project_uuid}"
            )
            async with session.get(f"{files_metadata_url}") as response:
                response.raise_for_status()
                list_of_files_enveloped = Envelope[list[FileMetaDataGet]].model_validate(await response.json())
                assert list_of_files_enveloped.data is not None  # nosec
            project_size_bytes += sum(file_metadata.file_size for file_metadata in list_of_files_enveloped.data)
...
async def get_download_link(app: web.Application, user_id: UserID, filelink: SimCoreFileLink) -> HttpUrl:
    """
    Raises:
        ClientError
        ValidationError
    ...
    """
    session, api_endpoint = _get_storage_client(app)
    url = (api_endpoint / f"locations/{filelink.store}/files" / urllib.parse.quote(filelink.path, safe="")).with_query(
        user_id=user_id
    )

    async with session.get(f"{url}") as response:
        response.raise_for_status()
        download: PresignedLink | None = Envelope[PresignedLink].model_validate(await response.json()).data
...
async def get_files_in_node_folder(
    ...
    files_metadata_url = (api_endpoint / "locations" / f"{_SIMCORE_LOCATION}" / "files" / "metadata").with_query(
        user_id=user_id, uuid_filter=s3_folder_path, expand_dirs="true"
    )
