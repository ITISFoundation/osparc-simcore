...
async def batch_service_metadata(
    ...
    *,
    ...
    ).where(tuple_(services_meta_data.c.key, services_meta_data.c.version).in_(keys_and_versions))

    async with pass_or_acquire_connection(get_asyncpg_engine(app), connection) as conn:
        result = await conn.execute(query)
        rows = result.fetchall()

        return {(row.key, row.version): ServiceMetadata.model_validate(row, from_attributes=True) for row in rows}
...
async def get_service_metadata(
    ...
    *,
    ...
    query = select(
        ...
        services_meta_data.c.version,
        services_meta_data.c.thumbnail,
    ).where(tuple_(services_meta_data.c.key, services_meta_data.c.version) == (key, version))
