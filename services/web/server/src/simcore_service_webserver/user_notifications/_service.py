...
async def list_user_notifications(app: web.Application, user_id: UserID, product_name: str) -> list[UserNotification]:
...
async def create_user_notification(app: web.Application, notification_data: UserNotificationCreate) -> None:
...
async def update_user_notification(
    ...
    repo = UserNotificationsRepository.create_from_app(app)
    return await repo.update_notification(user_id=user_id, notification_id=notification_id, update_data=update_data)
