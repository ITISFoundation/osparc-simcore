...
def map_to_scicrunch_error(rrid: str, error_code: int, message: str) -> ScicrunchError:
    ...
    assert status.HTTP_400_BAD_REQUEST <= error_code <= status.HTTP_511_NETWORK_AUTHENTICATION_REQUIRED, error_code  # nosec

    custom_error = ScicrunchError(details="Unexpected error in scicrunch.org", original_message=message)
    ...
    if error_code == web_exceptions.HTTPBadRequest.status_code:
        ...
        custom_error = InvalidRRIDError(msg_template=f"Did not find any '{rrid}'", original_message=message)

    elif error_code == web_exceptions.HTTPUnauthorized.status_code:
        custom_error = ScicrunchConfigError(
            details="osparc was not authorized to access scicrunch.org.Please check API access tokens.",
            original_message=message,
        )

    elif error_code >= status.HTTP_500_INTERNAL_SERVER_ERROR:  # scicrunch.org server error
