...
def _get_rclone_mount_command(
    ...
    command: list[str] = [
        ...
        "--config",
        config_file_path,
        f"--log-file=/tmp/rclone-debug{uuid4()}.log",
        "-vv",
        "mount",
...
class TrackedMount:
    ...
    async def start_mount(self) -> None:
        ...
        config_file_path = await self._cleanup_stack.enter_async_context(
            config_file(get_config_content(self.settings, self.mount_type))
        )
...
class RCloneMountManager:
    def __init__(self, app: FastAPI) -> None:
        self.app = app
        # keep track of all started mount commands via their pid and http endpoint, might need different ports for the http API
        # add rc-user and rc-password the the config stored here so that nobody can access without credentials

        self._started_mounts: dict[MountID, TrackedMount] = {}

    async def start_mount(self, remote_type: MountRemoteType) -> MountID:
        # create a mount via some configuration and keep track of it
        pass

    async def stop_mount(self, mount_id: MountID) -> None:
        pass
...
# TODO: be able to mange multiple sources to be mounted
