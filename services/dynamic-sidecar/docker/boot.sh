...
R_CLONE_CONTAINER_VERSION=$(rclone version | head -n1 | awk '{print $2}' | sed 's/^v//') && \
  echo "$INFO" "R_CLONE_CONTAINER_VERSION=${R_CLONE_CONTAINER_VERSION}" && \
  export R_CLONE_CONTAINER_VERSION
