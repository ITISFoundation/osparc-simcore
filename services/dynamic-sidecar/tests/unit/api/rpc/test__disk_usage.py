...
@pytest.fixture
def mock_environment(redis_service: RedisSettings, mock_environment: EnvVarsDict) -> EnvVarsDict:
    return mock_environment


async def test_get_state(app: FastAPI, rpc_client: RabbitMQRPCClient):
    usage = {"some_path": DiskUsage(total=ByteSize(0), used=ByteSize(0), free=ByteSize(0), used_percent=0)}
    settings: ApplicationSettings = app.state.settings

    result = await disk_usage.update_disk_usage(rpc_client, node_id=settings.DY_SIDECAR_NODE_ID, usage=usage)
