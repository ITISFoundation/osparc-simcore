...
from simcore_service_dynamic_sidecar.modules.rclone import _mount
from simcore_service_dynamic_sidecar.modules.rclone._config_provider import (
    MountRemoteType,
)
from simcore_service_dynamic_sidecar.modules.rclone._mount import (
...
def mock_config_file(config_path: Path, faker: Faker, mocker: MockerFixture) -> None:
    ...
    mocker.patch.object(_mount, "config_file", config_file)
...
@pytest.fixture
def application_settings(mock_environment: EnvVarsDict) -> ApplicationSettings:
...
async def s3_client(
    ...
    s3_settings = application_settings.DY_SIDECAR_R_CLONE_SETTINGS.R_CLONE_S3
...
@pytest.fixture
def bucket_name(application_settings: ApplicationSettings) -> S3BucketName:
    return TypeAdapter(S3BucketName).validate_python(
        application_settings.DY_SIDECAR_R_CLONE_SETTINGS.R_CLONE_S3.S3_BUCKET_NAME,
    )
...
@pytest.fixture
def mock_default_remote_control_host(mocker: MockerFixture) -> None:
    mocker.patch(
        "simcore_service_dynamic_sidecar.modules.rclone._mount._DEFAULT_REMOTE_CONTROL_HOST",
        "0.0.0.0",  # noqa: S104
    )
...
@pytest.fixture
async def tracked_mount(
    mock_default_remote_control_host: None,
    ...
    r_clone_local_mount_path: Path,
    vfs_cache_path: Path,
) -> AsyncIterable[TrackedMount]:
    tracked_mount = TrackedMount(
        application_settings,
        MountRemoteType.S3,
        rc_port=_MONITORING_PORT,
        remote_path=remote_path,
        local_mount_path=r_clone_local_mount_path,
        vfs_cache_path=vfs_cache_path,
    )
    await tracked_mount.setup()

    yield tracked_mount

    await tracked_mount.teardown()
...
# TODO:
# better real world tests
# use this to mount a folder like the node directly in a separate path on the sidecar and expose this
# to the users somehow
# so that we can use it form a jupyter-math
