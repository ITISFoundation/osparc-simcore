...
@pytest.mark.parametrize(
    ...
    [
        pytest.param([], _MAX_DEFAULT_METRICS_SCRAPE_INTERVAL, id="no_prometheus_queries"),
        pytest.param(
            [_DT_REF],
            _MAX_DEFAULT_METRICS_SCRAPE_INTERVAL,
        ...
        pytest.param(
            [_DT_REF + timedelta(seconds=i * 3) for i in range(_MAX_PROMETHEUS_SAMPLES)],
            3,
            id="average_over_prometheus_queries",
        ),
    ],
)
def test_get_user_services_scrape_interval(input_query_times: Sequence[datetime], expected: NonNegativeFloat):
