...
@pytest.fixture
def mock_outputs_manager() -> MagicMock:
    manager = MagicMock()
...
async def test_push_output_ports_success_does_not_touch_permissions(
    progress: AsyncMock,
    app: MagicMock,
    mock_outputs_manager: MagicMock,
    mock_post_sidecar_log_message: AsyncMock,
    mock_ensure_permissions: AsyncMock,
):
    mock_outputs_manager.wait_for_all_uploads_to_finish.side_effect = [None]

    await long_running_tasks.push_user_services_output_ports(progress, app, mock_outputs_manager)

    assert mock_outputs_manager.wait_for_all_uploads_to_finish.await_count == 1
    mock_ensure_permissions.assert_not_awaited()
    mock_outputs_manager.set_all_ports_for_upload.assert_not_called()
...
async def test_push_output_ports_fixes_permissions_and_retries(
    ...
    mock_outputs_manager.wait_for_all_uploads_to_finish.side_effect = [
    ...
    assert mock_outputs_manager.wait_for_all_uploads_to_finish.await_count == 2
    mock_ensure_permissions.assert_awaited_once_with(app.state.mounted_volumes)
    mock_outputs_manager.set_all_ports_for_upload.assert_called_once_with()
...
async def test_push_output_ports_raises_when_permission_fix_fails(
    ...
    mock_outputs_manager.wait_for_all_uploads_to_finish.side_effect = original_error
