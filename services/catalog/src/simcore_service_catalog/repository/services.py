...
class ServicesRepository(BaseRepository):
    ...
    async def create_or_update_service(
        ...
        async with self.db_engine.begin() as conn:
            ...
            # Use INSERT ON CONFLICT DO UPDATE to make this operation idempotent
            insert_stmt = (
                ...
                .on_conflict_do_update(
                    ...
                    set_=new_service.model_dump(exclude_unset=True, exclude={"key", "version"}),
                )
            ...
            # Batch insert/update all access rights in the same transaction
            for access_rights in new_service_access_rights:
                upsert_stmt = (
                    pg_insert(services_access_rights)
                    .values(**jsonable_encoder(access_rights, by_alias=True))
                    .on_conflict_do_update(
                        index_elements=[
    ...
    async def upsert_service_access_rights(self, new_access_rights: list[ServiceAccessRightsDB]) -> None:
        # Batch all access rights upserts in a single transaction for atomicity
        async with self.db_engine.begin() as conn:
