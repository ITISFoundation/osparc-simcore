...
def osparc_simcore_root_dir(project_slug_dir):
    ...
    assert root_dir and root_dir.exists(), "Did you renamed or moved the integration folder under api-gateway??"
    assert any(root_dir.glob("services/api-gateway")), "%s not look like rootdir" % root_dir
    return root_dir
